...
export function ChallengeBrowser({ maps, level }: { maps: MapEntry[]; level: number }) {
  ...
  async function checkMap(id: string) {
    ...
    try {
      const response = await fetch("/api/challenge/check", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ challengeMapId: id }),
      });
      const data = await response.json();
      if (!response.ok) {
        setMessages((current) => ({ ...current, [id]: data.error ?? "Unable to check your score." }));
      } else if (data.status === "level_up") {
        setMessages((current) => ({ ...current, [id]: `Level up! You reached Challenge Level ${data.level}.` }));
      } else if (data.status === "locked") {
        setMessages((current) => ({ ...current, [id]: `Locked — pass a Level ${data.requiredLevel} map first.` }));
      } else if (data.status === "already") {
        setMessages((current) => ({ ...current, [id]: "You already completed this map." }));
      } else if (data.status === "passed") {
        setMessages((current) => ({ ...current, [id]: data.points > 0 ? `Map completed. +${data.points} RHP.` : "Map completed. It is outside your current RHP rank range." }));
      } else {
        setMessages((current) => ({ ...current, [id]: "No passing score found. Beat the map in Rhythia and check again." }));
      }
    } catch {
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-border bg-surface/95 p-5 shadow-glow">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-muted">Your Challenge Level</p>
            <p className="mt-1 text-3xl font-semibold text-white">Level {level}</p>
            {level < MAX_LEVEL && <p className="mt-1 text-xs text-muted">Pass one Level {nextLevel} map to reach Level {nextLevel}.</p>}
          </div>
          ...
          <section key={mapLevel} className="rounded-3xl border border-border bg-surface/95 p-5 shadow-glow">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className={`inline-flex h-9 w-9 items-center justify-center rounded-xl text-sm font-bold ${isDone ? "bg-emerald-400/15 text-emerald-300" : isNext ? "bg-accent/15 text-accent" : "bg-white/5 text-muted"}`}>
                  {mapLevel}
                </span>
                <div>
                  <p className="text-sm font-semibold text-white">Challenge Level {mapLevel}</p>
                  <p className="text-xs text-muted">{isDone ? "Completed" : isNext ? "Your next level" : "Locked — complete earlier levels first"}</p>
                </div>
              </div>
              <span className="text-xs text-muted">{levelMaps.length} map{levelMaps.length === 1 ? "" : "s"}</span>
            </div>
            {levelMaps.length === 0 ? (
              <p className="mt-4 rounded-2xl border border-dashed border-border bg-background/50 p-4 text-sm text-muted">No maps in this level yet.</p>
            ) : (
                    ...
                    <article key={map.id} className="flex flex-col rounded-2xl border border-border bg-background/60 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-xs text-muted">{map.rating.toFixed(2)} rating · mapped by {map.mapperName ?? "Unknown"}</p>
                        </div>
                        {map.completion?.passed ? (
                          <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 text-xs font-semibold text-emerald-300"><CheckCircle2 size={13} /> Done</span>
                        ) : isLocked ? (
                          <span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-border bg-white/5 px-2.5 py-1 text-xs font-semibold text-muted"><Lock size={13} /> Locked</span>
                        ) : null}
                      </div>
                      ...
                      {message && <p className="mt-3 rounded-xl border border-border bg-background/70 p-3 text-sm text-muted">{message}</p>}
                    </article>
                  );
                })}
              </div>
