...
export function MapAdminSearch() {
  ...
  async function search() {
    ...
    setSearched(false);
    try {
      const response = await fetch(`/api/admin/maps/${encodeURIComponent(id)}`, { method: "GET" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Could not find this map.");
      setMap(data.map);
      setEditRating(data.map.rating != null ? String(data.map.rating) : "");
      setSearched(true);
    } catch (searchError) {
      setError(searchError instanceof Error ? searchError.message : "Could not find this map.");
      setSearched(true);
    } finally {
      setLoading(false);
    }
      ...
      <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
        ...
        >
          <input
            ...
            placeholder="Paste a map ID (uuid)…"
        ...
        <p className="mt-3 text-xs text-muted">
          Search an approved map by ID to view its details, change its rating, remove individual scores, or delete it.
        </p>
      </section>
        ...
        <div className="space-y-6">
          <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-wider ${STATUS_STYLES[map.status] ?? "border-border bg-white/5 text-muted"}`}>
                {map.status}
              </span>
              {map.isAutoImported && (
                <span className="rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-xs font-medium text-accent">Auto-imported</span>
              )}
            </div>
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              <div className="space-y-3 rounded-3xl border border-border bg-background/60 p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-accent">Submitted by</p>
                {map.submittedBy ? (
                  <>
                    <p className="text-sm text-white">{map.submittedBy.displayName ?? map.submittedBy.username}</p>
                    <p className="text-sm text-muted">@{map.submittedBy.profileHandle}</p>
                  </>
                ) : (
              ...
              <div className="space-y-3 rounded-3xl border border-border bg-background/60 p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-accent">Approved by</p>
                {map.reviewedBy ? (
                  <>
                    <p className="text-sm text-white">{map.reviewedBy.displayName ?? map.reviewedBy.username}</p>
                    <p className="text-sm text-muted">@{map.reviewedBy.profileHandle} · {formatDate(map.reviewedAt)}</p>
                  </>
                ) : (
            ...
            <div className="mt-4 flex flex-wrap items-end gap-4 rounded-3xl border border-border bg-background/60 p-5">
              <label className="block">
                <span className="text-xs font-semibold text-muted">Current rating</span>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  ...
                  className="mt-2 w-40 rounded-xl border border-border bg-background px-3 py-2 text-sm text-white outline-none transition focus:border-accent"
                />
              </label>
              <button
                onClick={saveRating}
              ...
              >
                <Star size={15} /> {saving ? "Saving..." : "Save rating"}
              </button>
            </div>
            ...
            <div className="mt-4 space-y-3 rounded-3xl border border-border bg-background/60 p-5">
              ...
              {map.sourceUrl && (
                <a href={map.sourceUrl} target="_blank" rel="noreferrer" className="break-all text-sm text-accent hover:text-white">Source: {map.sourceUrl}</a>
              )}
          ...
          <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
            <p className="text-sm font-semibold text-white">Scores ({map.completions.length})</p>
            <p className="mt-1 text-sm text-muted">Remove a player&apos;s score to claw back the RHP they earned from this map.</p>
            {map.completions.length === 0 ? (
              <p className="mt-4 text-sm text-muted">No scores recorded for this map.</p>
            ) : (
              <div className="mt-4 overflow-hidden rounded-2xl border border-border">
                {map.completions.map((completion) => (
                  <div key={completion.id} className="flex flex-wrap items-center gap-3 border-b border-border bg-background/60 px-4 py-3 last:border-0 sm:flex-nowrap">
                    <div className="min-w-0 flex-1">
                      <Link href={`/profile/${completion.user.profileHandle}`} className="truncate text-sm font-semibold text-white hover:text-accent">
                        {completion.user.displayName ?? completion.user.username}
                      </Link>
                      <p className="text-xs text-muted">
                        {completion.passed ? "Passed" : "Failed"} · {completion.points} RHP{completion.accuracy != null ? ` · ${completion.accuracy.toFixed(2)}%` : ""} · {formatDate(completion.createdAt)}
                      </p>
                    </div>
                    <button
                      disabled={removingId === completion.id}
                      onClick={() => removeCompletion(completion.id, completion.user.username)}
                      className="inline-flex items-center gap-2 rounded-full border border-red-400/40 px-3 py-1.5 text-xs font-semibold text-red-200 transition hover:bg-red-400/10 disabled:opacity-50"
                    >
                      <Trash2 size={13} /> {removingId === completion.id ? "Removing..." : "Remove score"}
                    </button>
                  </div>
                ))}
              </div>
          ...
          <section className="rounded-3xl border border-red-400/20 bg-red-400/5 p-6 shadow-glow">
            <p className="text-sm font-semibold text-white">Delete this map</p>
            <p className="mt-1 text-sm text-muted">
              This permanently removes the map and all its scores. This cannot be undone.
            </p>
            <button
              disabled={deleting}
              onClick={deleteMap}
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-red-500 px-5 py-2 text-sm font-semibold text-white transition hover:bg-red-400 disabled:opacity-50"
            >
              <Trash2 size={15} /> {deleting ? "Deleting..." : "Delete map"}
            </button>
          </section>
        </div>
      ) : null}
