...
export function TournamentLiveApp({ tournamentId }: { tournamentId: string }) {
  ...
  return <div className="ui-page space-y-6">
    <section className="overflow-hidden rounded-[2.3rem] border border-accent/20 bg-[radial-gradient(circle_at_10%_0%,rgba(124,143,240,.14),transparent_32%),linear-gradient(145deg,rgba(20,27,45,.97),rgba(8,12,22,.98))] p-7 shadow-glow sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-accent"><Trophy size={15} /> {data.tournament.status === "completed" ? "Tournament archive" : "Live tournament"}</p><h1 className="mt-2 text-4xl font-black tracking-[-0.04em] text-white">{data.tournament.name}</h1><p className="mt-2 text-sm text-muted">{data.mode} · event ID <span className="font-mono text-white">{data.tournament.id}</span> · {data.runtime?.targetPlayersPerSplit ?? "—"} players per split</p><p className="mt-1 text-xs text-muted">Canonical live address: <span className="font-mono text-white">/tournaments/{data.tournament.id}</span></p></div>{data.viewerSignup && <div className="rounded-2xl border border-white/10 bg-black/15 px-5 py-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-muted">Your status</p><p className={`mt-1 font-bold capitalize ${kicked ? "text-rose-200" : "text-white"}`}>{data.viewerEliminated ? "Eliminated" : data.viewerSignup.status} · {data.viewerSignup.split}</p>{data.runtime?.presence?.warningStage > 0 && !kicked && <p className="mt-1 text-xs text-amber-200">Inactivity warning stage {data.runtime.presence.warningStage}/3</p>}</div>}</div>
    </section>
