...
function FinalCard({ match, label }: { match: any; label: string }) {
  ...
  return <div className={`relative overflow-hidden rounded-[1.75rem] border p-5 ${live ? "border-fuchsia-400/35 bg-fuchsia-400/[0.07]" : complete ? "border-amber-300/30 bg-amber-300/[0.06]" : "border-white/10 bg-black/15"}`}>
    {live && <span className="absolute right-4 top-4 flex items-center gap-1.5 rounded-full border border-fuchsia-300/20 bg-fuchsia-300/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-fuchsia-100"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-fuchsia-300" />Live</span>}
    ...
    {match.map?.title && <div className="mt-3 rounded-xl border border-white/8 bg-black/10 px-3 py-2"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-muted">Final map</p><p className="mt-1 truncate text-sm font-bold text-white">{match.map.title}</p></div>}
