...
export function ScheduledMapPool({ tournamentId }: { tournamentId: string }) {
  ...
  const render = (maps: any[]) => <div className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-3">{maps.map((map: any) => <article key={map.mapId} className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-black/15 p-4"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="break-words font-bold text-white">{map.title}</p><p className="mt-1 break-words text-xs text-muted">{map.artist || "Unknown artist"} · {Number(map.rating).toFixed(2)} rating</p></div><span className="shrink-0 rounded-full bg-white/5 px-2 py-1 text-xs text-muted">{Number(map.sourcePlaycount ?? 0).toLocaleString()} plays</span></div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => action({ action: "vote", mapId: map.mapId, value: 1 })} className={`ui-button !px-3 !py-2 text-xs ${map.viewerVote === 1 ? "bg-emerald-400/20 text-emerald-100" : "bg-white/5"}`}><ThumbsUp size={13} /> {map.likes}</button><button onClick={() => action({ action: "vote", mapId: map.mapId, value: -1 })} className={`ui-button !px-3 !py-2 text-xs ${map.viewerVote === -1 ? "bg-rose-400/20 text-rose-100" : "bg-white/5"}`}><ThumbsDown size={13} /> {map.dislikes}</button><button onClick={() => { const reason = window.prompt("Why should this map be removed and replaced?"); if (reason) void action({ action: "report", mapId: map.mapId, reason }); }} className="ui-button !px-3 !py-2 text-xs bg-white/5"><Flag size={13} /> Report</button></div></article>)}</div>;
