...
export function GroupChatManager({ conversationId, currentUserId }: { conversationId?: string; currentUserId: string }) {
  ...
  function mountControls() {
    ...
    const header = Array.from(document.querySelectorAll("header")).find((item) => /members/.test(item.textContent ?? ""));
    ...
    rename.className = "rounded-xl border border-border px-3 py-2 text-xs font-semibold text-muted hover:text-white";
    rename.textContent = "Rename";
    rename.onclick = () => { setError(""); setOpen(true); void loadFriends(); };
    const add = document.createElement("button");
    add.className = "rounded-xl bg-accent px-3 py-2 text-xs font-semibold text-white";
    add.textContent = "Add people";
  ...
  async function saveName() {
    ...
    const response = await fetch(`/api/messages/conversations/${conversation.id}/name`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: value }) });
  ...
  async function addMembers() {
    ...
    const response = await fetch(`/api/messages/conversations/${conversation.id}/members`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userIds: selected }) });
    const data = await response.json().catch(() => ({}));
  ...
  const available = friends.filter((friend) => !conversation.memberIds.includes(friend.id));

  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-3xl border border-border bg-surface p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-accent">Group management</p><h2 className="mt-1 text-2xl font-black text-white">{conversation.name}</h2></div><button onClick={() => setOpen(false)} className="rounded-xl px-3 py-2 text-sm text-muted hover:bg-white/5 hover:text-white">Close</button></div><label className="mt-5 block text-sm font-semibold text-white">Group name<input value={name} onChange={(e) => setName(e.target.value)} maxLength={60} className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-white outline-none focus:border-accent" /></label><button onClick={() => void saveName()} className="mt-3 rounded-xl border border-accent/40 bg-accent/10 px-4 py-2 text-sm font-semibold text-accent">Save name</button><div className="mt-6 border-t border-border pt-5"><p className="text-sm font-semibold text-white">Add friends</p><div className="mt-3 max-h-56 space-y-2 overflow-y-auto">{available.length === 0 ? <p className="rounded-xl border border-dashed border-border p-4 text-sm text-muted">All of your friends are already in this group.</p> : available.map((friend) => <label key={friend.id} className="flex items-center gap-3 rounded-xl border border-border bg-background/40 p-3"><input type="checkbox" checked={selected.includes(friend.id)} onChange={(e) => setSelected((current) => e.target.checked ? [...current, friend.id] : current.filter((id) => id !== friend.id))} /><img src={friend.avatar ?? ""} alt="" className="h-9 w-9 rounded-full object-cover" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-white">{friend.displayName || friend.username}</span><span className="block truncate text-xs text-muted">@{friend.profileHandle}</span></span></label>)}</div><button onClick={() => void addMembers()} className="mt-3 rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-white">Add selected</button></div>{error && <p className="mt-4 rounded-xl border border-red-400/30 bg-red-400/10 p-3 text-sm text-red-200">{error}</p>}</div></div>;
}