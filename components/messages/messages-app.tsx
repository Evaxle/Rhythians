...
export function MessagesApp({ currentUserId, initialTargetHandle, initialConversationId }: { currentUserId: string; initialTargetHandle?: string; initialConversationId?: string }) {
  ...
  return <div className="flex h-[calc(100dvh-9rem)] min-h-[28rem] flex-col overflow-hidden rounded-3xl border border-border bg-surface/95 shadow-glow">
    <div className="flex min-h-0 flex-1">
      <aside className={`w-full shrink-0 flex-col border-r border-border md:flex md:w-[19rem] lg:w-[22rem] ${activeId && detail ? "hidden" : "flex"}`}>
