...
import type { ClipSourceType, ExternalClipSourceType } from "@/lib/clip-source";
...
const tabs: Array<{ value: ClipSourceType; label: string; hint: string }> = [
  ...
  { value: "medal", label: "Medal.tv", hint: "Paste a Medal.tv clip URL" },
];
...
type Metadata = { sourceType: ExternalClipSourceType; url: string; title: string; description: string; thumbnailUrl: string | null };

function sourceLabel(sourceType: ClipSourceType) {
  if (sourceType === "tiktok") return "TikTok video URL";
  if (sourceType === "youtube") return "YouTube video URL";
  if (sourceType === "twitch") return "Twitch clip URL";
  if (sourceType === "medal") return "Medal.tv clip URL";
  return "Video URL";
}

function placeholder(sourceType: ClipSourceType) {
  if (sourceType === "tiktok") return "https://www.tiktok.com/@user/video/...";
  if (sourceType === "youtube") return "https://www.youtube.com/watch?v=...";
  if (sourceType === "twitch") return "https://clips.twitch.tv/...";
  if (sourceType === "medal") return "https://medal.tv/games/rhythia/clips/...";
  return "";
}
...
export default function ClipSubmitForm() {
  ...
  const [metadataState, setMetadataState] = useState<"idle" | "processing" | "ready" | "error">("idle");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setMetadata(null);
      setMetadataState("processing");
      setMetadataError("");
      try {
        const response = await fetch("/api/clips/metadata", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sourceType, sourceUrl: sourceUrl.trim() }),
          signal: controller.signal,
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "Could not process that video URL.");
        setMetadata(data.metadata as Metadata);
        setMetadataState("ready");
      } catch (reason) {
        if (controller.signal.aborted) return;
        setMetadataState("error");
        setMetadataError(reason instanceof Error ? reason.message : "Could not process that video URL.");
      }
    }, 650);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (sourceType === "upload") {
      ...
      if (!sourceUrl.trim()) return setError(`Paste a ${sourceLabel(sourceType).toLowerCase()}.`);
      if (metadataState !== "ready" || !metadata) return setError("Wait for the video URL to finish processing before submitting.");
    }
    ...
    try {
      ...
      const response = await fetch("/api/clips/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: sourceType === "upload" ? title.trim() : undefined, songName: songName.trim() || null, description: sourceType === "upload" ? description.trim() : undefined, cameraMode: cameraMode || null, sourceType, sourceUrl: sourceUrl.trim() || undefined, storagePath, thumbnailPath }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Clip submission failed.");
      setSuccess("Clip submitted! It will appear once approved.");
      setTitle(""); setSongName(""); setDescription(""); setCameraMode(""); setVideoFile(null); setThumbnailFile(null); setSourceUrl(""); setMetadata(null); setMetadataState("idle");
      router.refresh();
    } catch (err) {
  ...
  const externalProcessing = sourceType !== "upload" && metadataState === "processing";
  const submitDisabled = loading || externalProcessing || (sourceType !== "upload" && metadataState !== "ready");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-border bg-surface/95 p-5 shadow-glow sm:p-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-muted">External video titles and descriptions are read from the source automatically before submission.</p><button type="submit" disabled={submitDisabled} className="inline-flex items-center justify-center rounded-full bg-accent px-6 py-3 text-sm font-semibold text-white transition hover:bg-accent2 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-muted disabled:opacity-70">{loading ? "Submitting…" : externalProcessing ? "Processing video…" : "Submit clip"}</button></div>
    </form>
  );
}
