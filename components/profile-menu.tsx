...
export function ProfileMenu({ user }: { user: ProfileMenuUser }) {
  ...
  const avatarUrl = avatarHash ? avatarHash.startsWith("http") ? avatarHash : user.discordId ? `https://cdn.discordapp.com/avatars/${user.discordId}/${avatarHash}.${avatarExtension}?size=128` : null : null;
  const showAvatar = Boolean(avatarUrl) && !avatarFailed;

  return <div ref={wrapperRef} className="relative" onMouseEnter={openMenu} onMouseLeave={scheduleClose}>
    <button onClick={() => setOpen((current) => !current)} aria-label="Account menu" aria-expanded={open} className="flex cursor-pointer items-center gap-2 rounded-full border border-border bg-white/5 px-2 py-1.5 text-sm text-white transition hover:border-accent/40">
      {showAvatar ? <img src={avatarUrl!} alt={`${user.username}'s avatar`} className="h-8 w-8 rounded-full object-cover" width={32} height={32} decoding="async" onError={() => setAvatarFailed(true)} /> : <span className="flex h-8 w-8 items-center justify-center rounded-full bg-accent text-xs font-bold text-white">{user.username.slice(0, 1).toUpperCase()}</span>}
    ...
    {open && <div className="animate-rise-in absolute right-0 z-30 mt-2 w-56 rounded-2xl border border-border bg-surface p-2 shadow-glow">
      <div className="border-b border-border px-3 py-2"><p className="truncate text-sm font-semibold text-white">{user.displayName ?? user.username}</p><p className="truncate text-xs text-muted">@{user.profileHandle}</p><p className="mt-1 inline-flex items-center gap-1 rounded-full border border-accent/30 bg-accent/10 px-2 py-0.5 text-xs font-semibold text-accent"><Trophy size={12} /> {user.rhp.toLocaleString()} RHP</p></div>
      ...
      <Link href="/rhythia/request" className="block rounded-xl px-3 py-2 text-sm text-muted transition hover:bg-white/5 hover:text-white">Rhythia verification</Link>
