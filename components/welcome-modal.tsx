...
export function WelcomeModal({ user, hasLinkedProfile, profileHandle }: { user: boolean; hasLinkedProfile: boolean; profileHandle: string | null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={dismiss} role="dialog" aria-modal="true" aria-label="Welcome to Rhythians">
      <div className="animate-modal-in relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-border bg-surface p-6 shadow-glow sm:p-8" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          <Link href="/knowledge" onClick={dismiss} className="rounded-2xl border border-border bg-background/70 p-4 transition hover:border-accent/40">
