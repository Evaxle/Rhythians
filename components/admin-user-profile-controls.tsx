...
const pointSystems = [["rhp", "RHP"], ["rpl", "RPL · Lock"], ["rps", "RPS · Spin"], ["rpv", "RPV · VR"], ["rbp", "RBP · Battles"]] as const;
...
export function AdminUserProfileControls({ userId }: { userId: string }) {
  ...
  async function toggle() {
    ...
    try {
      ...
      setPoints(Object.fromEntries(pointSystems.map(([key]) => [key, String(result.points?.[key] ?? 0)])));
    ...
    try {
      const response = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/levels`, {
        ...
        body: JSON.stringify({
          ...
          points: Object.fromEntries(pointSystems.map(([key]) => [key, Number(points[key] ?? 0)])),
        }),
      });
      const result = await response.json();
    ...
    {open && data && <div className="mt-4 space-y-4">
      <div className="rounded-2xl border border-border bg-background/60 p-4">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {pointSystems.map(([key, label]) => <label key={key} className="block"><span className="mb-1 block text-xs text-muted">{label}</span><input type="number" min={0} max={1000000} value={points[key] ?? "0"} onChange={(event) => setPoints((current) => ({ ...current, [key]: event.target.value }))} className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm text-white outline-none focus:border-accent" /></label>)}
        </div>
        <p className="mt-3 text-xs leading-5 text-muted">RPL, RPS, and RPV remain separate pools but contribute to RHP. RBP is battle-only and never contributes to RHP.</p>
      </div>
      <div className="rounded-2xl border border-border bg-background/60 p-4">
