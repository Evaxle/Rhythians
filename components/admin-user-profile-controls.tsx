...
export function AdminUserProfileControls({ userId }: { userId: string }) {
  ...
  const [data, setData] = useState<{ challengeLevel: number; categoryLevels: Array<{ category: string; level: number }>; profileTitle: string; profileTitleColor: string; canEditTitle: boolean } | null>(null);
  ...
  async function toggle() {
    ...
    try {
      ...
      setData(result);
      setMainLevel(String(result.challengeLevel));
      ...
      setError(error instanceof Error ? error.message : "Could not load profile controls.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveLevels() {
    ...
    try {
      const response = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/levels`, {
        ...
        body: JSON.stringify({ challengeLevel: Number(mainLevel), categoryLevels: Object.fromEntries(categories.map(([key]) => [key, Number(levels[key] ?? 0)])) }),
      });
      const result = await response.json();
      ...
      setError(error instanceof Error ? error.message : "Could not save levels.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function saveTitle() {
    ...
    try {
      ...
      setTitleColor(result.color ?? "#a78bfa");
    } catch (error) {
      setError(error instanceof Error ? error.message : "Could not save title.");
    } finally {
      setSaving(false);
    }
  }
      ...
      {open && data && (
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          <div className="rounded-2xl border border-border bg-background/60 p-4">
            ...
            <button onClick={saveLevels} disabled={saving} className="mt-4 rounded-full bg-accent px-4 py-2 text-xs font-semibold text-white transition hover:bg-accent2 disabled:opacity-50">{saving ? "Saving…" : "Save levels"}</button>
          </div>
          {data.canEditTitle && <div className="rounded-2xl border border-border bg-background/60 p-4">
            ...
            <div className="mt-3 flex items-center gap-2"><input type="color" value={titleColor} onChange={(event) => setTitleColor(event.target.value)} className="h-9 w-12 cursor-pointer rounded-lg border border-border bg-background p-1" /><input value={titleColor} onChange={(event) => setTitleColor(event.target.value)} maxLength={7} className="w-28 rounded-xl border border-border bg-background px-3 py-2 text-sm text-white outline-none focus:border-accent" /><span className="text-sm font-semibold" style={{ color: titleColor }}>{title || "Preview"}</span></div>
            <button onClick={saveTitle} disabled={saving} className="mt-4 rounded-full bg-accent px-4 py-2 text-xs font-semibold text-white transition hover:bg-accent2 disabled:opacity-50">{saving ? "Saving…" : "Save title"}</button>
          </div>}
        </div>
      )}
    </div>
