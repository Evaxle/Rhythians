...
export function SuggestionsForum({ signedIn }: { signedIn: boolean }) {
  ...
  return <div className="mx-auto w-full max-w-5xl space-y-6 px-4 py-6 sm:px-6">
    <section className="rounded-[2rem] border border-border bg-surface/95 p-6 shadow-glow sm:p-8">
    ...
    <section className="space-y-3">{items.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 p-10 text-center text-sm text-muted">No suggestions yet.</div> : items.map((item) => <article key={item.id} className="rounded-3xl border border-white/10 bg-surface/90 p-5 shadow-glow">
      <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap gap-2"><span className="rounded-full border border-accent/25 bg-accent/[0.08] px-2.5 py-1 text-[10px] font-bold text-accent">{item.category}</span><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] text-muted">{item.status}</span></div><h2 className="mt-3 text-xl font-bold text-white">{item.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted">{item.body}</p><p className="mt-3 text-xs text-muted">by <Link href={`/profile/${item.author.profileHandle}`} className="text-white hover:text-accent">{item.author.displayName ?? item.author.username}</Link> · {new Date(item.createdAt).toLocaleDateString()}</p></div></div>
