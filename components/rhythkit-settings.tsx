...
import { Download, RefreshCw, Wifi, Terminal, Play, Trash2 } from "lucide-react";
...
export function RhythKitSettings() {
  ...
  const getUrl = (path: string) => `http://127.0.0.1:${GAMES.find(game => game.id === active)?.port ?? 45872}${path}`;
  ...
  const testConnection = async () => {
    setTesting(active);
    try {
      await fetch(getUrl("/test-connection"), { method: "POST" });
    } finally {
      await check(active);
      await loadEvents();
      setTesting(null);
  ...
  const clearEvents = async () => {
    try {
      await fetch(getUrl("/debug/events"), { method: "DELETE" });
      setEvents([]);
    } catch { }
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 border-b border-border pb-3">
        {GAMES.map(game => {
          ...
          return <button key={game.id} onClick={() => setActive(game.id)} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition ${active === game.id ? "bg-accent text-white" : "border border-border bg-white/5 text-muted hover:text-white"}`}><span className={`h-2.5 w-2.5 rounded-full ${state === "connected" ? "bg-green-400" : state === "not_connected" ? "bg-red-400" : "bg-yellow-400"}`} />{game.name}</button>;
        })}
      </div>

      <div className="rounded-2xl border border-border bg-background/60 p-5 space-y-4">
        <div>
          <div className="flex items-center gap-3"><span className={`h-3 w-3 rounded-full ${current.state === "connected" ? "bg-green-400" : current.state === "not_connected" ? "bg-red-400" : "bg-yellow-400"}`} /><p className="font-semibold text-white">{current.state === "connected" ? "Rhythians connected" : current.state === "not_connected" ? "Not connected" : "Checking..."}</p></div>
          <p className="mt-2 text-sm text-muted">{current.state === "connected" ? `Connected as ${current.username ?? "your account"}.` : `Install and start RhythKit for ${GAMES.find(game => game.id === active)?.name ?? "this game"} to establish the local connection.`}</p>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-xl border border-border bg-white/5 p-4"><p className="text-xs uppercase tracking-[0.2em] text-muted">Game</p><p className="mt-2 font-semibold text-white">{current.gameRunning ? "Running" : "Not running"}</p><p className="mt-1 text-xs text-muted">{current.game ?? GAMES.find(game => game.id === active)?.name}</p></div>
