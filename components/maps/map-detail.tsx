    ...
    <section className="overflow-hidden rounded-3xl border border-border bg-surface/95 shadow-glow">
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className={`mt-6 grid gap-3 ${data.isRanked ? "sm:grid-cols-2 lg:grid-cols-6" : "sm:grid-cols-2 lg:grid-cols-4"}`}>
          <div className="rounded-2xl border border-accent/30 bg-accent/[0.08] p-4"><p className="text-xs uppercase tracking-wider text-muted">Rankability</p><p className="mt-1 text-3xl font-black text-white">{data.rankability.toFixed(2)}<span className="text-sm font-semibold text-muted"> / 5.00</span></p><p className="mt-1 text-[11px] leading-4 text-muted">Competitive balance and ranking suitability.</p><button type="button" onClick={() => setShowRankabilityDetails((v) => !v)} className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-accent">{showRankabilityDetails ? <ChevronUp size={14} /> : <ChevronDown size={14} />}{showRankabilityDetails ? "Hide details" : "View more details"}</button></div>
    ...
    <section className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow sm:p-8">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.challengeFits.map((item) => {
        ...
        return <div key={item.category} className={`rounded-2xl border p-4 ${assigned ? "border-fuchsia-400/40 bg-fuchsia-400/[0.08]" : "border-white/10 bg-black/15"}`}>
          <div className="flex items-start justify-between gap-3"><div><p className="font-bold text-white">{item.label}</p>{assigned && <p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-fuchsia-300">Assigned challenge category</p>}</div><span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-xs font-black text-white">Level {item.suggestedLevel}</span></div>
          ...
          <p className="mt-3 text-[11px] leading-5 text-muted">{item.reason}</p>
          <div className="mt-3 flex flex-wrap gap-1.5">{item.metrics.slice(0, 3).map((metric) => <span key={metric.label} className="rounded-full border border-white/10 bg-black/15 px-2 py-1 text-[10px] text-muted">{metric.label}: {metric.value}</span>)}</div>
        </div>;
      })}</div>
    </section>
