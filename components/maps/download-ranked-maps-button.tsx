...
export function DownloadRankedMapsButton({ rankLabel }: { rankLabel: string }) {
  ...
  async function handleDownload(mode: "ranked" | "ranked-legacy" | "legacy") {
    setBusy(true); setOpen(false); setProgress(null);
    try {
      ...
      while (true) {
        ...
        while (buffer.length >= 5) {
          ...
          if (type === 1) { const item = JSON.parse(new TextDecoder().decode(payload)) as { downloaded: number; total: number }; setProgress({ downloaded: item.downloaded, total: item.total }); }
