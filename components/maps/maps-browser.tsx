...
export function MapsBrowser({ maps, rankInfo, userRhp, showLegacy: externalShowLegacy, modeScores, modeTab = "all" }: Props) {
  ...
  const visibleForTab = (map: MapEntry) => {
    if (map.rating == null) return false;
    if (map.isLegacy) return activeMode == null && isMapInRankRange(map.rating, rankInfo.index);
    return activeMode ? eligibleFor(map, activeMode) : Boolean(map.isRanked && isMapInRankRange(map.rating, rankInfo.index));
  };
  const filtered = useMemo(() => {
    ...
    return list;
  }, [maps, showAll, showLegacy, query, modeTab, activeMode, rankInfo.index, modeRanks.lock.index, modeRanks.spin.index, modeRanks.vr.index]);
  const visibleMaps = filtered.slice(0, visibleCount);
  const eligibleCount = maps.filter((map) => modeTab === "legacy" ? map.isLegacy && (showAll || visibleForTab(map)) : visibleForTab(map)).length;
  ...
  return <div className="space-y-5">
    ...
    {filtered.length === 0 ? <div className="rounded-[1.8rem] border border-dashed border-white/10 bg-black/10 p-10 text-center text-sm text-muted">No maps match this eligibility view.</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{visibleMaps.map((map) => {
      const mapScores = modeScores[titleKey(map.title)] ?? { lock: 0, spin: 0, vr: 0 }; const displayRank = map.rating != null ? RANKS[map.rankIndex] : null; const displayRankColor = displayRank?.color ?? map.rankColor; const displayRankInfo = getRankInfo(displayRank?.minRhp ?? rankInfo.minRhp); const eligible = map.isLegacy ? true : activeMode ? eligibleFor(map, activeMode) : visibleForTab(map); const duration = lengthLabel(map.length); const message = messages[map.id]; const mapDifficulty = mapRankLabel(map.rating, map.rankIndex, map.rankName);
