...
export function MapsBrowser({ maps, rankInfo, userRhp, currentUserId }: { maps: MapEntry[]; rankInfo: RankInfo; userRhp: number; currentUserId: string | null }) {
  ...
  const rankMapCount = maps.filter((map) => map.rating != null && isMapInRankRange(map.rating, rankInfo.index)).length;
  ...
  async function downloadRankMaps() {
    setDownloading(true);
    try {
      const response = await fetch("/api/maps/download-rank");
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        throw new Error(body?.error ?? "Unable to download your rank's maps.");
      }
      ...
      setMessages((current) => ({ ...current, __download: { tone: "err", text: error instanceof Error ? error.message : "Unable to download your rank's maps." } }));
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-border bg-surface/95 p-5 shadow-glow sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => void downloadRankMaps()} disabled={downloading || rankMapCount === 0} className="inline-flex items-center gap-2 rounded-full border border-accent/40 bg-accent/10 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-accent/20 disabled:opacity-50">
            <Download size={15} /> {downloading ? "Preparing ZIP..." : `Download ${rankMapCount} rank maps`}
          </button>
      ...
      {messages.__download && <p className="rounded-2xl border border-red-400/30 bg-red-400/10 p-3 text-sm text-red-200">{messages.__download.text}</p>}
