  ...
  return <div className="space-y-5">
    ...
    {filtered.length === 0 ? <div className="rounded-[1.8rem] border border-dashed border-white/10 bg-black/10 p-10 text-center text-sm text-muted">No maps match this eligibility view.</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{visibleMaps.map((map) => {
      ...
      return <article key={map.id} className="group flex min-h-[390px] flex-col overflow-hidden rounded-[1.75rem] border bg-gradient-to-br from-white/[0.045] to-black/15 shadow-glow transition hover:-translate-y-1" style={{ borderColor: eligible ? `${displayRankColor}55` : "rgba(244,63,94,.22)" }}>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5 text-[10px] text-muted">{map.noteCount != null && <span className="rounded-full border border-white/8 bg-black/10 px-2 py-1">{map.noteCount.toLocaleString()} notes</span>}{duration && <span className="rounded-full border border-white/8 bg-black/10 px-2 py-1">{duration}</span>}<span className="rounded-full border border-white/8 bg-black/10 px-2 py-1" style={{ color: displayRankColor }}>{map.isLegacy ? `${displayRank?.name ?? map.rankName} · Legacy` : displayRank?.name ?? map.rankName}</span></div></Link>
        <div className="mt-auto flex items-center justify-between gap-2 pt-4"><p className="text-[10px] leading-4 text-muted">{map.isLegacy ? "Legacy archive · analysis reference only." : eligible ? activeMode ? `Eligible for ${MODE_META[activeMode].short}.` : "Eligible for your current overall rank." : "Outside the selected eligibility range."}</p>{map.isRanked && <button type="button" onClick={() => void checkMap(map.id)} disabled={busyId === map.id} className="shrink-0 rounded-xl bg-accent px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">{busyId === map.id ? "Checking…" : "Check"}</button>}</div>{message && <p className="mt-3 rounded-xl border border-accent/25 bg-accent/[0.07] p-3 text-[11px] leading-5 text-accent">{message}</p>}</div>
      </article>;
    })}</div>}
