...
export function MapsBrowser({ maps, rankInfo, userRhp, showLegacy: externalShowLegacy, modeTab = "all" }: Props) {
  ...
  useEffect(() => setVisibleCount(PAGE_SIZE), [modeTab, showLegacy]);
  ...
  return <div className="space-y-5 rounded-[2rem] p-1 sm:p-2" style={{ background: `radial-gradient(circle at top, ${rankInfo.color}24 0%, ${rankInfo.color}0d 28%, transparent 62%)` }}>
    <section className="rounded-[1.8rem] border p-5 shadow-glow" style={{ borderColor: `${rankInfo.color}45`, background: `linear-gradient(135deg, ${rankInfo.color}18, rgba(0,0,0,.16))` }}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-3"><RankIcon rank={rankInfo} size={44} /><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">{modeTab === "legacy" ? "Legacy archive" : "All-ranks map catalog"}</p><p className="mt-1 text-lg font-bold text-white"><span style={{ color: rankInfo.color }}>{labelRank(rankInfo)}</span> · {filtered.length} maps</p><p className="mt-1 text-xs text-muted">Your {userRhp.toLocaleString()} RHP · all map ranks are always visible</p></div></div>
      </div>
    ...
    {filtered.length === 0 ? <div className="rounded-[1.8rem] border border-dashed border-white/10 bg-black/10 p-10 text-center text-sm text-muted">No maps match this filter view.</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{visibleMaps.map((map) => {
      ...
      return <article key={map.id} className="group flex min-h-[390px] flex-col overflow-hidden rounded-[1.75rem] border bg-gradient-to-br from-white/[0.045] to-black/15 shadow-glow transition hover:-translate-y-1" style={{ borderColor: `${displayRankColor}55` }}>
        ...
        {map.isRanked && map.maxRewards && <div className="mt-4 rounded-2xl border border-accent/30 bg-accent/[0.08] p-4"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-muted">Maximum points for a balanced pass</p><div className="mt-3 grid grid-cols-3 gap-2"><div className="rounded-xl border border-white/10 bg-black/15 p-2.5 text-center"><p className="text-[10px] font-bold text-muted">LOCK</p><p className="mt-1 text-xl font-black text-white">{map.maxRewards.lock}</p><p className="text-[10px] font-bold text-accent">RPL</p></div><div className="rounded-xl border border-white/10 bg-black/15 p-2.5 text-center"><p className="text-[10px] font-bold text-muted">SPIN</p><p className="mt-1 text-xl font-black text-white">{map.maxRewards.spin}</p><p className="text-[10px] font-bold text-accent">RPS</p></div><div className="rounded-xl border border-white/10 bg-black/15 p-2.5 text-center"><p className="text-[10px] font-bold text-muted">VR</p><p className="mt-1 text-xl font-black text-white">{map.maxRewards.vr}</p><p className="text-[10px] font-bold text-accent">RPV</p></div></div></div>}
