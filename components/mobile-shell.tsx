...
export function MobileShell({ children, canReview, canAdmin, user }: MobileShellProps) {
  ...
  useEffect(() => {
    ...
    const previous = { htmlOverflow: html.style.overflow, bodyOverflow: body.style.overflow, bodyPosition: body.style.position, bodyWidth: body.style.width, bodyHeight: body.style.height };
    html.style.overflow = "hidden";
    body.style.overflow = "hidden";
    body.style.position = "fixed";
    body.style.width = "100%";
    body.style.height = "100dvh";
    return () => { html.style.overflow = previous.htmlOverflow; body.style.overflow = previous.bodyOverflow; body.style.position = previous.bodyPosition; body.style.width = previous.bodyWidth; body.style.height = previous.bodyHeight; };
  }, []);
  ...
  const shell = <div className={`mobile-app fixed inset-0 z-[1000] h-[100dvh] w-screen overflow-hidden bg-[#060914] text-white mobile-nav-${navSide} ${prefs.compactNav ? "mobile-nav-compact" : ""} ${prefs.reduceMotion ? "mobile-reduced-motion" : ""}`}>
    <div className="mobile-app-topbar fixed inset-x-0 top-0 z-50 flex h-[calc(var(--mobile-topbar-height)+env(safe-area-inset-top))] items-end border-b border-white/10 bg-[#080c18]/96 pb-2 pt-[env(safe-area-inset-top)] shadow-xl backdrop-blur-2xl">
      <div className={`flex w-full min-w-0 items-center justify-between gap-2 px-3 ${navSide === "right" ? "pr-[calc(var(--mobile-nav-width)+10px)]" : "pl-[calc(var(--mobile-nav-width)+10px)]"}`}><div className="min-w-0"><p className="truncate text-sm font-bold text-white">Rhythians</p><p className="text-[10px] text-muted">Mobile</p></div><div className="flex min-w-0 items-center gap-1.5"><Link href="/mobile/search" aria-label="Search" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-muted"><Search size={16} /></Link>{user && prefs.showTopbarProfile ? <Link href={`/mobile/profile/${user.profileHandle}`} aria-label="Open profile" className="flex min-w-0 max-w-[44vw] items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-2 py-1">{user.avatar && user.discordId ? <img src={user.avatar.startsWith("http") ? user.avatar : `https://cdn.discordapp.com/avatars/${user.discordId}/${user.avatar}.${user.avatar.startsWith("a_") ? "gif" : "png"}?size=64`} alt="" className="h-7 w-7 shrink-0 rounded-full object-cover" /> : <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-accent text-[10px] font-bold">{user.username.slice(0, 1).toUpperCase()}</span>}<span className="min-w-0 truncate text-xs font-semibold text-white">{user.displayName ?? user.username}</span></Link> : <Link href="/mobile/settings" aria-label="Account" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-muted"><UserRound size={16} /></Link>}</div></div>
    </div>
    <aside className={`mobile-app-nav fixed bottom-0 top-[calc(var(--mobile-topbar-height)+env(safe-area-inset-top))] z-40 flex w-[var(--mobile-nav-width)] min-w-0 flex-col border-white/10 bg-[#080c18]/98 pb-[calc(.55rem+env(safe-area-inset-bottom))] pt-1.5 shadow-2xl backdrop-blur-2xl ${navSide === "right" ? "right-0 border-l" : "left-0 border-r"}`}><Link href="/mobile" aria-label="Rhythians home" className="mx-auto mb-1.5 grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-xl bg-gradient-to-br from-accent to-indigo-400 shadow-lg ring-1 ring-white/10"><img src="/favicon.ico" alt="Rhythians" className="h-full w-full object-cover" /></Link><nav className="mobile-nav-scroll flex min-h-0 flex-1 flex-col items-center gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 overscroll-contain">{items.map(([href, label, Icon]) => { const path = href.split("?")[0]; const active = path === "/" ? pathname === "/mobile" : pathname === `/mobile${path}` || pathname.startsWith(`/mobile${path}/`); return <Link key={href} href={`/mobile${href === "/" ? "" : href}`} aria-label={label} className={`mobile-nav-item group flex w-full shrink-0 flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-1.5 text-[9px] font-semibold transition ${active ? "bg-accent/20 text-white ring-1 ring-accent/30" : "text-muted hover:bg-white/[0.06] hover:text-white"}`}><Icon size={17} strokeWidth={active ? 2.4 : 1.9} /><span className="max-w-full truncate leading-tight">{label}</span></Link>; })}</nav></aside>
    <section className={`mobile-app-main fixed bottom-0 top-[calc(var(--mobile-topbar-height)+env(safe-area-inset-top))] min-w-0 overflow-y-auto overflow-x-hidden overscroll-contain ${navSide === "right" ? "left-0 right-[var(--mobile-nav-width)]" : "left-[var(--mobile-nav-width)] right-0"}`}><main className="mobile-page mx-auto min-h-full w-full max-w-2xl min-w-0 overflow-x-hidden px-2.5 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-2.5 sm:px-4">{children}</main></section>
  </div>;
