...
export function ReportsManager({
  ...
  const filteredReports = useMemo(() => {
    const byType = reports.filter((report) => report.targetType === tab);
  ...
  const openCount = (type: "user" | "clip") => reports.filter((r) => r.targetType === type && r.status === "open").length;
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "banned" ? (
        ...
        <div className="rounded-3xl border border-border bg-surface/95 p-10 text-center text-sm text-muted">
          {normalizedQuery ? "No reports match your search." : `No ${tab === "user" ? "user" : "post"} reports yet.`}
        </div>
      ) : (
