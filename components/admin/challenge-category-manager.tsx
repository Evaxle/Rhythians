...
export function ChallengeCategoryManager({ isOwner = false }: { isOwner?: boolean }) {
  ...
  async function assign(mapId: string, value: string) {
    ...
    try {
      const response = await fetch("/api/admin/challenge/manage", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fromTab: tab, tab, mapId, level: nextLevel }) });
      const data = await response.json();
