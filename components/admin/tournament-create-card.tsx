...
export function TournamentCreateCard() {
  ...
  return <section className="rounded-3xl border border-accent/20 bg-gradient-to-br from-accent/[0.08] via-surface/95 to-surface/95 p-6 shadow-glow sm:p-7">
    ...
    <div className="mt-6 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_130px_190px_230px_auto]">
      ...
      <label className="space-y-1.5"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted">Mode</span><select value={mode} onChange={(event) => changeMode(event.target.value)} className="w-full rounded-2xl border border-white/10 bg-[#101629] px-4 py-3 text-sm text-white"><option value="1v1">1v1</option><option value="2v2">2v2</option><option value="3v3">3v3</option></select></label>
      <label className="space-y-1.5"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted">Players per split</span><select value={playersPerSplit} onChange={(event) => setPlayersPerSplit(Number(event.target.value))} className="w-full rounded-2xl border border-white/10 bg-[#101629] px-4 py-3 text-sm text-white">{playerTargets.map((target, index) => <option key={target} value={target}>{target} players · {TEAM_TARGETS[index]} teams{index === 0 ? " · Base" : ""}</option>)}</select></label>
