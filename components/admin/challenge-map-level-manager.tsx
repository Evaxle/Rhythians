...
export function ChallengeMapLevelManager({ initialMaps, initialHasMore = false }: { initialMaps: ChallengeMap[]; initialHasMore?: boolean }) {
  const [maps, setMaps] = useState(initialMaps);
  const [hasMore, setHasMore] = useState(initialHasMore);
  const [nextOffset, setNextOffset] = useState(initialHasMore ? PAGE_SIZE : null as number | null);
  ...
  async function loadMore() {
    ...
    try {
      const params = new URLSearchParams({ tab: "challenge", offset: String(nextOffset) });
      const response = await fetch(`/api/admin/challenge/manage?${params.toString()}`, { cache: "no-store" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Could not load more maps.");
      const incoming = Array.isArray(data.maps) ? data.maps as ChallengeMap[] : [];
      setMaps((current) => {
        const known = new Set(current.map((map) => map.id));
        return [...current, ...incoming.filter((map) => !known.has(map.id))];
      });
      setHasMore(Boolean(data.hasMore));
      setNextOffset(typeof data.nextOffset === "number" ? data.nextOffset : null);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "Could not load more maps.");
    } finally {
