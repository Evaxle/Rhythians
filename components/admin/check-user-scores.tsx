...
export function AdminCheckUserScores() {
  ...
  async function check(payload: Record<string, unknown>) {
    if (payload.userId && !window.confirm(`Rebuild the best ranked-map pass for every mode for ${user?.displayName ?? user?.username}?`)) return;
    if (payload.all && !window.confirm("Recheck every linked user's best ranked-map pass for each game mode and recalculate RPL, RPS, RPV, and RHP from map analysis and miss balance? Unlinked users will be skipped.")) return;
    setBusy(true);
    setError("");
    setMessage("");
    try {
      ...
      if (data.mode === "all") {
        ...
        setMessage(`Rechecked ${data.usersChecked} of ${data.totalUsers} users (${data.linkedUsers} linked, ${data.skippedUnlinked} unlinked skipped). Found ${data.foundScores} best ranked-map mode passes, recorded ${data.newlyCompleted} new mode completions, and changed RHP by ${data.awardedPoints} net positive points.${failureText}`);
      } else {
        setMessage(`Rechecked ${data.mapsChecked} ranked-map mode passes for ${data.user?.displayName ?? data.user?.username ?? "user"}; found ${data.foundScores} best passes, recorded ${data.newlyCompleted} new mode completions, and changed RHP by ${data.awardedPoints} net positive points.`);
      }
    } catch (checkError) {
