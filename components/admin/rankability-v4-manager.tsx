    ...
    <div className="mt-6 space-y-4">
      {catalog?.maps.map((map) => { const rankability = map.rankability; const current = map.analysisStatus === "analyzed"; return <article key={map.id} className="rounded-2xl border border-border bg-background/50 p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
          <div className="flex min-w-0 flex-1 gap-3">{map.imageUrl ? <img src={map.imageUrl} alt="" className="h-16 w-28 shrink-0 rounded-xl object-cover"/> : <div className="h-16 w-28 shrink-0 rounded-xl border border-border bg-black/20"/>}<div className="min-w-0"><h3 className="truncate font-semibold text-white">{map.title}</h3><p className="mt-1 truncate text-xs text-muted">{map.artist ?? "Unknown artist"} · {map.mapperName ?? "Unknown mapper"}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px]"><span className="rounded-full border border-border px-2 py-1 capitalize text-muted">{map.sourceStatus}</span><span className={`rounded-full border px-2 py-1 ${current ? "border-emerald-400/30 text-emerald-300" : "border-amber-400/30 text-amber-300"}`}>{map.analysisStatus}</span>{map.rating != null && <span className="rounded-full border border-border px-2 py-1 text-white">Difficulty {map.rating.toFixed(2)}</span>}</div></div></div>
          <div className="min-w-[210px] rounded-2xl border border-accent/25 bg-accent/[0.08] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted">Rankability v{rankability?.version ?? 4}</p><p className="mt-1 text-3xl font-black text-white">{rankability ? rankability.score.toFixed(2) : "—"}<span className="text-sm text-muted"> / 5.00</span></p><p className="mt-1 text-xs font-semibold text-accent">{rankability?.verdict ?? "Analyze map to score"}</p>{rankability && <p className="mt-1 text-[10px] text-muted">Confidence {Math.round(rankability.confidence * 100)}%</p>}</div>
        ...
        {rankability && <div className="mt-4 border-t border-border pt-4"><p className="text-sm leading-6 text-muted">{rankability.summary}</p><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{rankability.factors.map((factor) => <div key={factor.key} className="rounded-xl border border-border bg-black/10 p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-white">{factor.label}</p><p className="text-xs font-black text-accent">{factor.score.toFixed(2)}/5</p></div><p className="mt-2 text-[11px] leading-5 text-muted">{factor.reason}</p></div>)}</div>{rankability.limitations.length > 0 && <div className="mt-3 rounded-xl border border-amber-400/20 bg-amber-400/[0.06] p-3"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-amber-200">Main limitations</p><div className="mt-2 space-y-1">{rankability.limitations.map((item) => <p key={item} className="text-[11px] leading-5 text-muted">• {item}</p>)}</div></div>}</div>}
      </article>; })}
    </div>
