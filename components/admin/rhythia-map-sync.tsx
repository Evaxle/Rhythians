...
export function RhythiaMapSync() {
  ...
  async function analyzeRankedCatalog() {
    ...
    for (;;) {
      ...
      if (!data.processed || !data.stats?.pending) return { analyzed, failed, recalculatedUsers, stats: data.stats };
    }
  }
  ...
  async function sync(status: "RANKED" | "UNRANKED" | "LEGACY") {
    ...
    try {
      ...
      if (status === "RANKED") {
        setMessage(`Ranked sync complete: ${count} maps checked, ${data.created} added, ${data.updated} updated, ${data.promoted} promoted. Starting analyzer v3...`);
        const analysis = await analyzeRankedCatalog();
        setMessage(`Ranked sync and analysis complete: ${count} maps checked, ${analysis.stats?.analyzed ?? analysis.analyzed}/${analysis.stats?.total ?? count} currently analyzed, ${analysis.stats?.failed ?? analysis.failed} failed, ${analysis.recalculatedUsers} affected player recalculations.`);
      } else {
