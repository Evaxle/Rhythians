...
export function RhythiaMapSync() {
  ...
  async function sync(status: "RANKED" | "UNRANKED") {
    setBusy(status);
    setMessage("");
    setError("");
    try {
      ...
      const count = status === "RANKED" ? data.ranked : data.unranked;
      setMessage(`${status === "RANKED" ? "Ranked" : "Unranked"} sync complete: ${count} Rhythia maps checked, ${data.created} added, ${data.updated} updated, ${data.promoted} promoted.`);
    } catch (syncError) {
