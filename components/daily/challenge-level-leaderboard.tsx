    ...
    <section className="space-y-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        {rows.length === 0 ? <p className="p-8 text-sm text-muted">No Challenge players yet.</p> : rows.map((row) => {
          ...
          return <div key={row.userId} className={`grid grid-cols-[2.5rem_minmax(0,1fr)_auto] items-center gap-3 border-b border-border px-4 py-3 last:border-0 ${current ? "bg-accent/10" : "bg-background/60"}`}>
