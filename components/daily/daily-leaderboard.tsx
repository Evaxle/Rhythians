  ...
  return (
    <section className="space-y-4">
        ...
        {RANKS.map((r, index) => (
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition ${
              selected === index ? "text-white" : "text-muted hover:text-white"
            }`}
            style={
              selected === index
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        {rows.length === 0 ? (
          <p className="p-8 text-sm text-muted">No one in this rank has beaten a daily map yet. Beat today&apos;s map to start your streak!</p>
        ) : (
          rows.map((row) => {
