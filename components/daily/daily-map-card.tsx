...
type CheckResult = { ok: true; status: "beat" | "already" | "not_beat"; points: number; streak?: number } | { ok: false; error: string };
...
export function DailyMapCard({ dailyMap, initialBeat, userRhp, streak, rankName, randomMaps }: { dailyMap: DailyMapData; initialBeat: Beat | null; userRhp: number; streak: number; rankName: string; randomMaps: RandomMap[] }) {
  ...
  async function runCheck(): Promise<CheckResult> {
    try {
      ...
      if (data.status === "beat") return { ok: true, status: "beat", points: 0, streak: data.streak };
  ...
  function applyResult(result: CheckResult) {
    ...
    if (result.status === "beat") { setBeat({ points: 0, accuracy: null, misses: null }); if (result.streak != null) setCurrentStreak(result.streak); setState("found"); setMessage("Daily map detected. Your streak has been updated."); }
    else if (result.status === "already") { setState("found"); setMessage("You already completed today's daily map."); }
  ...
  return <div className="space-y-5">
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_290px]">
      <section className="ui-card rounded-[2rem] p-5 shadow-glow sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><CalendarDays size={16} /> Daily streak map</p><p className="mt-2 text-sm text-muted">{dailyMap.artist ?? "Unknown artist"}</p><h2 className="mt-1 truncate text-2xl font-semibold text-white">{dailyMap.title}</h2><p className="mt-1 text-xs text-muted">Mapped by {dailyMap.mapperName ?? "Unknown"}</p></div><span className="inline-flex shrink-0 items-center gap-1 rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1 text-sm font-semibold text-amber-300">Rhythia {dailyMap.starRating.toFixed(2)}★</span></div>
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-black/15 p-3"><p className="flex items-center gap-1.5 text-xs uppercase tracking-[0.16em] text-muted"><Gauge size={11} /> Rhythia difficulty</p><p className="mt-1.5 text-base font-semibold text-white">{difficultyLabel}</p></div><div className="rounded-2xl border border-white/10 bg-black/15 p-3"><p className="flex items-center gap-1.5 text-xs uppercase tracking-[0.16em] text-muted"><Music2 size={11} /> Notes</p><p className="mt-1.5 text-base font-semibold text-white">{dailyMap.noteCount?.toLocaleString() ?? "—"}</p></div><div className="rounded-2xl border border-white/10 bg-black/15 p-3"><p className="flex items-center gap-1.5 text-xs uppercase tracking-[0.16em] text-muted"><Timer size={11} /> Length</p><p className="mt-1.5 text-base font-semibold text-white">{lengthLabel}</p></div><div className="rounded-2xl border border-white/10 bg-black/15 p-3"><p className="flex items-center gap-1.5 text-xs uppercase tracking-[0.16em] text-muted"><Clock size={11} /> Date</p><p className="mt-1.5 text-base font-semibold text-white">{dateLabel.split(",")[0]}</p></div></div>
        ...
        {beat ? <div className="mt-4 flex items-center gap-3 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 px-4 py-3"><CheckCircle2 size={18} className="shrink-0 text-emerald-300" /><div><p className="text-sm font-semibold text-emerald-300">Completed</p><p className="mt-1 text-sm text-emerald-200/90">Today&apos;s streak completion is recorded. RHP, RPL, RPV and RPS come only from analyzed point-eligible ranked-map clears.</p></div></div> : <div className="mt-4 flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><Clock size={18} className="shrink-0 text-muted" /><div><p className="text-sm font-semibold text-white">Not beaten yet</p><p className="mt-1 text-sm text-muted">Beat this map in Rhythia with a passing score, then check again to continue your daily streak.</p></div></div>}
