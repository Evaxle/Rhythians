  ...
  return <div className="space-y-5">
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_290px]">
      <section className="ui-card rounded-[2rem] p-5 shadow-glow sm:p-6">
        ...
        {beat ? <div className="mt-4 flex items-center gap-3 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 px-4 py-3"><CheckCircle2 size={18} className="shrink-0 text-emerald-300" /><div><p className="text-sm font-semibold text-emerald-300">Completed</p><p className="mt-1 text-sm text-emerald-200/90">Today&apos;s streak completion is recorded. Rank points come from analyzed ranked-map clears and the daily mode quest boost above.</p></div></div> : <div className="mt-4 flex items-center gap-3 rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><Clock size={18} className="shrink-0 text-muted" /><div><p className="text-sm font-semibold text-white">Not beaten yet</p><p className="mt-1 text-sm text-muted">Beat this map in Rhythia with a passing score, then check again to continue your daily streak.</p></div></div>}
