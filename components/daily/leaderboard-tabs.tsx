...
function SectionTabs({ selected, onSelect, initialRankIndex }: { selected: Section; onSelect: (section: Section) => void; initialRankIndex: number }) {
  return <div className="flex flex-wrap gap-2">
    {(["global", "my-rank", "lock", "spin", "vr"] as const).map((section) => (
      <button key={section} type="button" onClick={() => onSelect(section)} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${selected === section ? "border-accent/50 bg-accent/15 text-white" : "border-border bg-white/5 text-muted hover:text-white"}`}>
...
function RankPicker({ selected, onSelect }: { selected: number; onSelect: (rank: number) => void }) {
  return <div className="flex flex-wrap gap-2">
    {RANKS.map((rank, index) => <button key={rank.name} type="button" onClick={() => onSelect(index)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${selected === index ? "text-white" : "text-muted hover:text-white"}`} style={selected === index ? { borderColor: `${rank.color}88`, backgroundColor: `${rank.color}22`, color: rank.color } : { borderColor: "var(--border, #2a2a3a)" }}>{rank.name}</button>)}
  </div>;
}
...
export function LeaderboardTabs({
  ...
  rankedCameraLeaderboards,
  challengeLevelLeaderboard,
  currentUserId,
  initialRankIndex = 0,
}: {
  dailyLeaderboards: DailyLeaderboardRow[][];
  dailyCameraLeaderboards: CameraLeaderboards<DailyLeaderboardRow>;
  challengeLeaderboards: ChallengeRow[][];
  rankedCameraLeaderboards: CameraLeaderboards<ChallengeRow>;
  ...
  const renderDaily = () => {
    ...
    const rows = dailyCameraLeaderboards[dailySection][dailyRank] ?? [];
    return <section className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{dailySection.toUpperCase()} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">All ranks · {dailySection.toUpperCase()}</h2><p className="mt-2 text-sm text-muted">Only users with the {dailySection.toUpperCase()} camera-mode tag appear here.</p></div><RankPicker selected={dailyRank} onSelect={setDailyRank} /><DailyLeaderboard leaderboards={dailyCameraLeaderboards[dailySection]} currentUserId={currentUserId} initialRankIndex={dailyRank} />{rows.length === 0 ? null : null}</section>;
  };
  ...
  const renderRanked = () => {
    ...
    return <section className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{rankedSection.toUpperCase()} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">All ranks · {rankedSection.toUpperCase()}</h2><p className="mt-2 text-sm text-muted">Only users with the {rankedSection.toUpperCase()} camera-mode tag appear here.</p></div><RankPicker selected={rankedRank} onSelect={setRankedRank} /><ChallengeLeaderboard leaderboards={rankedCameraLeaderboards[rankedSection]} currentUserId={currentUserId} initialRankIndex={rankedRank} /></section>;
  };
  ...
  return <div className="space-y-6">
    ...
    {tab === "daily" ? <section className="space-y-6"><SectionTabs selected={dailySection} onSelect={setDailySection} initialRankIndex={initialRankIndex} />{renderDaily()}</section> : tab === "ranked" ? <section className="space-y-6"><SectionTabs selected={rankedSection} onSelect={setRankedSection} initialRankIndex={initialRankIndex} />{renderRanked()}</section> : <ChallengeLevelLeaderboard rows={challengeLevelLeaderboard} currentUserId={currentUserId} />}
  </div>;
}
