...
function CameraLeaderboard<T extends DailyLeaderboardRow | ChallengeRow>({ mode, rank, rows, currentUserId, type }: { mode: CameraMode; rank: number; rows: T[][]; currentUserId: string | null; type: "daily" | "ranked" }) {
  const rankInfo = RANKS[rank] ?? RANKS[0];
  const modeLabel = mode === "lock" ? "Camera Lock" : mode === "spin" ? "Camera Spin" : "VR";
  const Leaderboard = type === "daily" ? DailyLeaderboard : ChallengeLeaderboard;
  return <section className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{modeLabel} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">{rankInfo.name} · {modeLabel}</h2><p className="mt-2 text-sm text-muted">Players with the {modeLabel} tag, ranked only within the {rankInfo.name} rank.</p></div><RankPicker selected={rank} onSelect={() => {}} /><Leaderboard leaderboards={rows} currentUserId={currentUserId} initialRankIndex={rank} hideRankPicker /></section>;
}
...
export function LeaderboardTabs({ dailyLeaderboards, dailyCameraLeaderboards, challengeLeaderboards, rankedCameraLeaderboards, challengeLevelLeaderboard, currentUserId, initialRankIndex = 0 }: { dailyLeaderboards: DailyLeaderboardRow[][]; dailyCameraLeaderboards: CameraLeaderboards<DailyLeaderboardRow>; challengeLeaderboards: ChallengeRow[][]; rankedCameraLeaderboards: CameraLeaderboards<ChallengeRow>; challengeLevelLeaderboard: ChallengeLevelRow[]; currentUserId: string | null; initialRankIndex?: number }) {
  ...
  const renderDaily = () => {
    ...
    return <section className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{dailySection === "lock" ? "Camera Lock" : dailySection === "spin" ? "Camera Spin" : "VR"} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">{RANKS[dailyRank]?.name ?? "Rank"} · {dailySection === "lock" ? "Camera Lock" : dailySection === "spin" ? "Camera Spin" : "VR"}</h2><p className="mt-2 text-sm text-muted">Only users with the {dailySection === "lock" ? "Camera Lock" : dailySection === "spin" ? "Camera Spin" : "VR"} tag appear here.</p></div><RankPicker selected={dailyRank} onSelect={setDailyRank} /><DailyLeaderboard leaderboards={dailyCameraLeaderboards[dailySection]} currentUserId={currentUserId} initialRankIndex={dailyRank} hideRankPicker /></section>;
  };
  ...
  const renderRanked = () => {
    ...
    return <section className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{rankedSection === "lock" ? "Camera Lock" : rankedSection === "spin" ? "Camera Spin" : "VR"} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">{RANKS[rankedRank]?.name ?? "Rank"} · {rankedSection === "lock" ? "Camera Lock" : rankedSection === "spin" ? "Camera Spin" : "VR"}</h2><p className="mt-2 text-sm text-muted">Only users with the {rankedSection === "lock" ? "Camera Lock" : rankedSection === "spin" ? "Camera Spin" : "VR"} tag appear here.</p></div><RankPicker selected={rankedRank} onSelect={setRankedRank} /><ChallengeLeaderboard leaderboards={rankedCameraLeaderboards[rankedSection]} currentUserId={currentUserId} initialRankIndex={rankedRank} hideRankPicker /></section>;
  };
