...
export function LeaderboardTabs({ dailyLeaderboards, dailyCameraLeaderboards, challengeLeaderboards, rankedCameraLeaderboards, challengeLevelLeaderboard, currentUserId, initialRankIndex = 0 }: { dailyLeaderboards: DailyLeaderboardRow[][]; dailyCameraLeaderboards: CameraLeaderboards<DailyLeaderboardRow>; challengeLeaderboards: ChallengeRow[][]; rankedCameraLeaderboards: CameraLeaderboards<ChallengeRow>; challengeLevelLeaderboard: ChallengeLevelRow[]; currentUserId: string | null; initialRankIndex?: number }) {
  ...
  const renderDaily = () => {
    if (dailySection === "global") return <DailyLeaderboard key="daily-global" leaderboards={[globalDailyRows]} currentUserId={currentUserId} initialRankIndex={0} hideRankPicker />;
    if (dailySection === "my-rank") return <DailyLeaderboard key="daily-my-rank" leaderboards={dailyLeaderboards} currentUserId={currentUserId} initialRankIndex={initialRankIndex} />;
    const modeLabel = dailySection === "lock" ? "Camera Lock" : dailySection === "spin" ? "Camera Spin" : "VR";
    const selectedRank = RANKS[dailyRank] ?? RANKS[0];
    return <section key={`daily-camera-${dailySection}`} className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{modeLabel} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">{selectedRank.name} · {modeLabel}</h2><p className="mt-2 text-sm text-muted">Only users with the {modeLabel} tag appear here, ranked within the selected rank.</p></div><RankPicker selected={dailyRank} onSelect={setDailyRank} /><DailyLeaderboard key={`daily-${dailySection}-${dailyRank}`} leaderboards={dailyCameraLeaderboards[dailySection]} currentUserId={currentUserId} initialRankIndex={dailyRank} hideRankPicker /></section>;
  };

  const renderRanked = () => {
    if (rankedSection === "global") return <ChallengeLeaderboard key="ranked-global" leaderboards={[globalRankedRows]} currentUserId={currentUserId} initialRankIndex={0} hideRankPicker />;
    if (rankedSection === "my-rank") return <ChallengeLeaderboard key="ranked-my-rank" leaderboards={challengeLeaderboards} currentUserId={currentUserId} initialRankIndex={initialRankIndex} />;
    ...
    return <section key={`ranked-camera-${rankedSection}`} className="space-y-4"><div><p className="text-sm uppercase tracking-[0.24em] text-accent">{modeLabel} leaderboard</p><h2 className="mt-1 text-2xl font-semibold text-white">{selectedRank.name} · {modeLabel}</h2><p className="mt-2 text-sm text-muted">Only users with the {modeLabel} tag appear here, ranked within the selected rank.</p></div><RankPicker selected={rankedRank} onSelect={setRankedRank} /><ChallengeLeaderboard key={`ranked-${rankedSection}-${rankedRank}`} leaderboards={rankedCameraLeaderboards[rankedSection]} currentUserId={currentUserId} initialRankIndex={rankedRank} hideRankPicker /></section>;
  };
