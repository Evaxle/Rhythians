...
import { Check, Download, Loader2, Swords, Trophy, UserRound } from "lucide-react";
...
function Avatar({ player, large = false }: { player: any; large?: boolean }) {
  ...
  return player.avatar ? <img src={player.avatar} alt={player.username} className={`${size} rounded-full border border-border object-cover`} /> : <div className={`${size} flex items-center justify-center rounded-full border border-border bg-accent/10 text-accent`}><UserRound size={large ? 28 : 19} /></div>;
}
...
export function BattleMatchApp({ matchId }: { matchId: string }) {
  ...
  async function load() {
    ...
    if (response.ok) setData(result); else setError(result.error ?? "Match not found.");
    setLoading(false);
  }

  useEffect(() => { load(); const timer = setInterval(load, 2500); return () => clearInterval(timer); }, [matchId]);
  useEffect(() => { if (!data?.match?.startedAt || data.match.status !== "active") return; setCountdown(3); const timer = setInterval(() => setCountdown((value) => Math.max(0, value - 1)), 1000); return () => clearInterval(timer); }, [data?.match?.startedAt, data?.match?.status]);
