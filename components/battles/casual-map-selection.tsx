...
export function CasualMapSelection({ data, matchId, onRefresh }: { data: any; matchId: string; onRefresh: () => Promise<void> }) {
  ...
  }, [customOpen, query, matchId]);

  async function select(mode: string, mapId?: string) {
    setSaving(true);
    setError("");
    try {
      const response = await fetch(`/api/battles/matches/${encodeURIComponent(matchId)}/map-selection`, {
        method: "PATCH",
  ...
  return <section className="rounded-[2rem] border border-accent/20 bg-surface/95 p-6 shadow-glow">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-3">{options.map((option: any) => {
      const selected = data.match.mapId === option.mapId;
      return <button key={option.id} disabled={saving} onClick={() => void select(modes[option.bucket] ?? "middle")} className={`group overflow-hidden rounded-3xl border text-left transition ${selected ? "border-accent bg-accent/10" : "border-white/10 bg-background/40 hover:-translate-y-1 hover:border-accent/30"}`}>
        <div className="relative aspect-[4/3] overflow-hidden bg-white/5">{option.imageUrl ? <img src={option.imageUrl} alt="" className="h-full w-full object-cover transition group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-muted">No image</div>}<span className="absolute left-3 top-3 rounded-full bg-black/65 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-white">{labels[option.bucket] ?? option.bucket}</span></div>
        <div className="p-4"><p className="truncate text-lg font-semibold text-white">{option.title}</p><p className="mt-1 truncate text-xs text-muted">{option.artist ?? "Unknown artist"}{option.rating != null ? ` · ${Number(option.rating).toFixed(2)}★` : ""}</p>{selected && <p className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-accent"><Check size={13} /> Selected</p>}</div>
      </button>;
    })}</div>
    ...
    {saving && <p className="mt-4 flex items-center gap-2 text-sm text-muted"><Loader2 className="animate-spin" size={16} />Selecting map...</p>}
