...
export function ReportButton({
  ...
  if (status === "done") {
    return (
      <p className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-sm text-emerald-300">
        Report submitted. Thanks for keeping Rhythians safe.
      </p>
    );
  }
      ...
      {open ? (
        <form onSubmit={handleSubmit} className="space-y-3 rounded-3xl border border-border bg-background/70 p-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {status === "busy" ? "Submitting..." : "Submit report"}
            </button>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="rounded-full border border-border px-4 py-2 text-sm text-muted transition hover:text-white"
            >
              Cancel
            </button>
          </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full border border-border bg-white/5 px-4 py-2 text-sm text-muted transition hover:border-red-400/40 hover:text-red-200"
        >
