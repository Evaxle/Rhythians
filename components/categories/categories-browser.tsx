...
import { BarChart3, Map as MapIcon, Swords, Trophy } from "lucide-react";
...
export function CategoriesBrowser({ levels, stats, maps, currentUserId, challengeMaps, challengeLevel, isAdmin, defaultTab = "maps" }: { levels: Level[]; stats: Stats[]; maps: Record<Category, MapEntry[]>; currentUserId: string | null; challengeMaps: ChallengeMapEntry[]; challengeLevel: number; isAdmin: boolean; defaultTab?: "maps" | "leaderboards" | "stats" | "challenge" }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap gap-2">
      ...
      <button type="button" onClick={() => { setTab("challenge"); setMapView("challenge"); }} className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition ${tab === "challenge" ? "bg-accent text-white" : "border border-border bg-white/5 text-muted hover:border-accent/40 hover:text-white"}`}><Swords size={16} /> Challenge</button>
    </div>
    ...
    {tab === "challenge" && <ChallengeBrowser maps={challengeMaps} level={challengeLevel} isAdmin={isAdmin} />}
  </div>;
}
