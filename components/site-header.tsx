...
const links = [
  ...
  ["/path", "Path"],
  ["/maps", "Maps"],
  ["/categories?tab=challenge", "Challenge"],
  ["/online", "Online"],
...
export async function SiteHeader() {
  ...
  const extraLinks = [
    ...(user ? [["/messages", "Messages"] as const] : []),
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#080b14]/82 shadow-[0_10px_40px_rgba(0,0,0,0.18)] backdrop-blur-2xl">
      <div className="mx-auto flex max-w-[1700px] items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-5 lg:px-7 2xl:px-9">
        <Link href="/" className="group flex shrink-0 items-center gap-2.5 rounded-2xl px-1.5 py-1.5 sm:gap-3">
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-center gap-0.5 xl:flex">
          {links.map(([href, label]) => (
            <Link key={href} href={href} className="rounded-xl px-2.5 py-2 text-[11px] font-medium text-muted hover:bg-white/[0.05] hover:text-white 2xl:px-3 2xl:text-xs">
              {label}
            </Link>
          ))}
          {extraLinks.map(([href, label]) => (
            <Link key={href} href={href} className={`rounded-xl px-2.5 py-2 text-[11px] font-medium hover:bg-white/[0.05] hover:text-white 2xl:px-3 2xl:text-xs ${label === "Admin" || label === "Review" ? "text-accent" : "text-muted"}`}>
              {label}
            </Link>
          ))}
        </nav>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
          <details className="relative xl:hidden">
            ...
            <nav className="absolute right-0 mt-2 grid w-64 gap-1 rounded-2xl border border-white/10 bg-[#101629]/95 p-2 shadow-2xl backdrop-blur-2xl">
              {[...links, ...extraLinks].map(([href, label]) => (
                <Link key={href} href={href} className={`rounded-xl px-3.5 py-2.5 text-sm font-medium hover:bg-white/[0.06] hover:text-white ${label === "Admin" || label === "Review" ? "text-accent" : "text-muted"}`}>
                  {label}
                </Link>
              ))}
            </nav>
          ...
          <Link href="/search" aria-label="Search" className="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 text-sm text-muted hover:border-white/20 hover:bg-white/[0.07] hover:text-white sm:px-3.5">
            <Search size={16} />
          ...
          {user ? (
            ...
            <Link href="/login" className="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl bg-accent px-3.5 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(124,143,240,0.2)] hover:bg-accent2 sm:px-4">
