      ...
      {loading ? <section className="rounded-3xl border border-border bg-surface/70 p-8 text-sm text-muted shadow-glow">Loading connected Rhythia profiles and settings...</section> : visible.length === 0 ? <section className="rounded-3xl border border-dashed border-border bg-surface/70 p-12 text-center shadow-glow"><Trophy className="mx-auto text-muted" size={38} /><p className="mt-4 text-sm font-semibold text-white">No {mode === "lock" ? "camera lock" : "camera spin"} settings published yet</p></section> : <section className="space-y-5">
        {visible.map((entry) => {
          ...
          return <article key={entry.id} className="group relative overflow-hidden rounded-3xl border bg-surface/95 shadow-glow transition duration-300 hover:-translate-y-0.5 hover:shadow-2xl" style={{ borderColor: `${rank.color}30`, background: `radial-gradient(circle at 100% 0%, ${rank.color}16, transparent 45%), rgba(15,20,34,0.95)` }}><div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(420px,48%)]"><div className="min-w-0 p-6 sm:p-7 lg:p-8">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-border bg-background/50 p-3"><p className="text-[10px] uppercase tracking-[0.16em] text-muted">Rhythians global rank</p><p className="mt-1 text-lg font-bold text-white">{entry.rhythiansGlobalRank ? `#${entry.rhythiansGlobalRank.toLocaleString()}` : "Unavailable"}</p></div><div className="rounded-2xl border border-border bg-background/50 p-3"><p className="text-[10px] uppercase tracking-[0.16em] text-muted">Rhythia global rank</p><p className="mt-1 text-lg font-bold text-white">{entry.globalRank ? `#${entry.globalRank.toLocaleString()}` : "Unavailable"}</p></div><div className="rounded-2xl border border-border bg-background/50 p-3"><div className="flex items-center gap-2"><RankIcon rank={rank} size={30} /><div><p className="text-[10px] uppercase tracking-[0.16em] text-muted">Player classification</p><p className="text-sm font-semibold" style={{ color: entry.rhythianRankColor || rank.color }}>{entry.rhythianRank || (rank.isExpert ? "Expert" : `${rank.name} ${rank.tier}`)}</p></div></div></div><div className="rounded-2xl border border-border bg-background/50 p-3"><p className="text-[10px] uppercase tracking-[0.16em] text-muted">Rhythians points</p><p className="mt-1 text-sm font-semibold text-white">{Number(entry.rhp ?? 0).toLocaleString()} RHP</p></div></div>
          ...
          </div><div className="relative overflow-hidden bg-black"><LoopingVideoPreview src={entry.videoUrl} title={`${displayName} ${mode} settings preview`} /><span className="pointer-events-none absolute bottom-3 left-3 rounded-full border border-white/10 bg-black/60 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white/80 backdrop-blur">Muted · looping preview</span></div></div></article>;
        })}
      </section>}
    </div>
  );
