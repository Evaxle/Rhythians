...
export async function getApprovedMaps(includeAll: boolean, userId: string | null) {
  ...
  return {
    rankInfo,
    maps: visible.map((map) => ({ id: map.id, title: map.title, artist: map.artist, description: map.description, mapFileUrl: map.mapFileUrl, imageUrl: map.imageUrl, rating: map.rating, mapperName: map.mapperName, noteCount: map.noteCount, length: map.length, submittedBy: map.submittedBy, reviewedBy: map.reviewedBy, completion: stateMap.get(map.id) ?? null })),
  };
}
...
export async function getMapLeaderboard(mapId: string) {
  ...
  const completions = await prisma.challengeMapCompletion.findMany({
    where: { challengeMapId: mapId },
    include: { user: { select: { id: true, username: true, displayName: true, profileHandle: true, avatar: true } } },
    orderBy: [{ passed: "desc" }, { accuracy: "desc" }, { points: "desc" }, { createdAt: "asc" }],
  });
  return {
    map: { id: map.id, title: map.title, rating: map.rating },
    entries: completions.map((entry, index) => ({ position: index + 1, userId: entry.user.id, username: entry.user.username, displayName: entry.user.displayName, profileHandle: entry.user.profileHandle, avatar: entry.user.avatar, passed: entry.passed, accuracy: entry.accuracy, points: entry.points, createdAt: entry.createdAt })),
  };
}
