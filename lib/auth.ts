...
const SESSION_COOKIE_NAME =
  process.env.SESSION_COOKIE_NAME ?? "rhythians_session";
...
async function userFromRhythKit(requestHeaders: Headers) {
  ...
  const installation = await getRhythKitInstallation(
    new Request("https://rhythians.local", {
...
export const getSessionUser = cache(async function getSessionUser() {
  ...
  if (!sessionToken) {
    ...
    if (!isOwner(bearerUser)) {
      ...
      if (bearerUser.suspendedUntil && bearerUser.suspendedUntil > new Date())
        return null;
    }
    return bearerUser;
  }
...
export function hasPermission(
  user: { roles: Array<any> } | null,
  permission: string,
) {
  if (!user) return false;
  const permissions = new Set(
    user.roles.flatMap((roleRecord: any) =>
      roleRecord.role.permissions.map((rp: any) => rp.permission.name),
    ),
  );
...
export async function createSession(userId: string) {
  ...
  const expiresAt = new Date(
    Date.now() +
      Number(process.env.SESSION_EXPIRES_DAYS ?? 30) * 24 * 60 * 60 * 1000,
  );
