...
export type RankedMapLeaderboard = {
  ...
  rating: number;
  rankability: number;
  rankabilityDetails: RankabilityBreakdown;
  challengeFits: ChallengeCategoryFit[];
...
async function loadMap(mapId: string) {
  ...
  if (/^\d+$/.test(mapId)) {
    ...
    const stored = numeric > 0x7fffffff ? numeric - 0x100000000 : numeric;
    return prisma.challengeMap.findFirst({
      where: { sourceBeatmapId: stored },
      select,
...
async function challengeAssignmentsFor(map: {
  id: string;
  sourceBeatmapId: number | null;
  mapFileUrl: string;
  title: string;
  ...
  const main = await prisma.$queryRawUnsafe<Array<{ level: number }>>(
  ...
  if (main[0]) {
    assignments.push({
      category: "challenge",
      label: CHALLENGE_FIT_LABELS.challenge,
      level: main[0].level,
    });
  }

  const category = await prisma.$queryRawUnsafe<Array<{ category: string; level: number }>>(
    `SELECT "category"::text AS "category","level"
     FROM "CategoryMap"
     WHERE "status"='approved'
       AND (
         ("sourceBeatmapId" IS NOT NULL AND "sourceBeatmapId"=$1)
         OR "mapFileUrl"=$2
  ...
  for (const row of category) {
    if (
      ...
      row.category === "stream" ||
      row.category === "tech" ||
      row.category === "off_grid" ||
      row.category === "vibro"
    ) {
      assignments.push({
        category: row.category,
        label: CHALLENGE_FIT_LABELS[row.category],
        level: row.level,
      });
    }
  }
...
export async function getRankedMapDetail(mapId: string): Promise<RankedMapLeaderboard | null> {
  ...
  if (
    !map ||
    !map.mapFileUrl ||
    (map.status !== "approved" && map.status !== "legacy") ||
    map.rating == null
  ) {
    return null;
  }
...
export async function upsertRankedMapScore(mapId: string, userId: string, score: ScoreWrite) {
  ...
  const existing = await prisma.challengeMapCompletion.findUnique({
    where: { challengeMapId_userId: { challengeMapId: detail.mapId, userId } },
  });

  const shouldReplace =
    !existing ||
    score.passed !== existing.passed
      ? score.passed
      : score.points > existing.points ||
        (score.points === existing.points && (score.accuracy ?? -1) > (existing.accuracy ?? -1));
...
export async function getRankedMapLeaderboard(
  mapId: string,
  _selectedRank: number | null,
  limit = 100,
): Promise<RankedMapLeaderboard | null> {
  const detail = await getRankedMapDetail(mapId);
  if (!detail) return null;
  if (!detail.isRanked) return detail;

  const completions = await prisma.challengeMapCompletion.findMany({
    where: { challengeMapId: detail.mapId, passed: true },
    include: {
      user: {
        select: {
          id: true,
  ...
  const eligible = completions
    .filter((entry) => getRankInfo(entry.user.rhp).index === detail.rankIndex)
    .sort(
      (a, b) =>
  ...
  const rows = eligible.map((entry, index) => ({
    position: index + 1,
    userId: entry.user.id,
    username: entry.user.username,
    displayName: entry.user.displayName,
    profileHandle: entry.user.profileHandle,
