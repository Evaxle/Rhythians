...
async function loadMap(mapId: string) {
  const direct = await prisma.challengeMap.findUnique({ where: { id: mapId }, select: { id: true, title: true, artist: true, description: true, mapFileUrl: true, imageUrl: true, rating: true, mapperName: true, noteCount: true, length: true, sourceBeatmapId: true, sourceUrl: true, status: true } });
  if (direct) return direct;
  if (/^\d+$/.test(mapId)) return prisma.challengeMap.findFirst({ where: { sourceBeatmapId: Number(mapId) }, select: { id: true, title: true, artist: true, description: true, mapFileUrl: true, imageUrl: true, rating: true, mapperName: true, noteCount: true, length: true, sourceBeatmapId: true, sourceUrl: true, status: true }, orderBy: { createdAt: "desc" } });
  return null;
}

export async function getRankedMapDetail(mapId: string): Promise<RankedMapLeaderboard | null> {
  const map = await loadMap(mapId);
  if (!map || map.status !== "approved" || map.rating == null || !map.mapFileUrl) return null;
