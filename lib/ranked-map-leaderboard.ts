...
export async function getRankedMapDetail(mapId: string): Promise<RankedMapLeaderboard | null> {
  ...
  if (!isRanked && !map.isAutoImported) return null;
  ...
  return { mapId: map.id, title: map.title, artist: map.artist, description: map.description, mapFileUrl: map.mapFileUrl, imageUrl: map.imageUrl, rating, rankIndex: rank.index, rankName: isRanked ? rank.name : "Unranked", rankColor: rank.color, rangeMin: rank.rangeMin, rangeMax: rank.rangeMax, mapperName: map.mapperName, noteCount: map.noteCount, length: map.length, sourceBeatmapId: map.sourceBeatmapId, sourceUrl: map.sourceUrl, rows: [], isRanked };
}
...
export async function getRankedMapLeaderboard(mapId: string, selectedRank: number | null, limit = 100): Promise<RankedMapLeaderboard | null> {
  ...
  if (!detail || !detail.isRanked) return detail;
