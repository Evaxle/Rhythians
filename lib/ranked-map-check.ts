...
async function alreadyAwarded(userId: string, mapId: string) {
  return Boolean(await prisma.rhpTransaction.findFirst({
    where: {
      userId,
      reason: "ranked_map",
      description: { startsWith: `Completed ranked map [${mapId}]:` },
    },
    select: { id: true },
  }));
}
...
export async function checkRankedMap(userId: string, mapId: string) {
  ...
  const map = await prisma.challengeMap.findUnique({
    where: { id: mapId },
    select: { id: true, title: true, rating: true, length: true, isAutoImported: true, status: true },
  });

  if (!map || !map.isAutoImported || map.status !== "approved" || map.rating == null) {
    return { status: "not_available" as const, points: 0 };
  }
  ...
  try {
    ...
    return { status: "error" as const, points: 0 };
  }
  ...
  await prisma.$transaction([
    ...
    prisma.rhpTransaction.create({
      data: {
        ...
        amount: points,
        reason: "ranked_map",
        description: `Completed ranked map [${map.id}]: ${map.title} (${map.rating.toFixed(2)})`,
      },
    }),
  ]);
