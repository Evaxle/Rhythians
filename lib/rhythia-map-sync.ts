...
function ratingForMap(map: SyncedRhythiaMap) { return fairRatingFromStars(Number.isFinite(map.starRating ?? NaN) ? map.starRating ?? 0 : 0); }
...
export async function syncRhythiaMaps(status?: RhythiaMapStatus) {
  ...
  for (const currentStatus of statuses) {
    ...
    for (const map of maps) {
      ...
      const reviewerNote = isUnranked ? UNRANKED_MARKER : null;
      if (!existing) {
        await prisma.challengeMap.create({ data: { title, artist: artistFromTitle(title), description: null, mapFileUrl, imageUrl: map.image, requestedRating: rating, rating, mapperName: map.ownerUsername, noteCount: map.noteCount, length: map.length, sourceBeatmapId, sourceUrl, isAutoImported: true, submittedById: importer.id, status: currentStatus === "LEGACY" ? "legacy" : "approved", reviewerNote, reviewedById: currentStatus === "LEGACY" ? importer.id : importer.id, reviewedAt: new Date() } });
        results.created += 1;
        continue;
      }
      ...
      if (currentStatus === "LEGACY") { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "legacy", reviewerNote: null, reviewedById: importer.id, reviewedAt: new Date() } }); results.updated += 1; }
      else if (currentStatus === "RANKED") { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "approved", reviewerNote: null, reviewedById: importer.id, reviewedAt: new Date() } }); results.promoted += 1; }
      else { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "approved", reviewerNote: UNRANKED_MARKER, reviewedById: importer.id, reviewedAt: new Date() } }); results.updated += 1; }
    }
  }
