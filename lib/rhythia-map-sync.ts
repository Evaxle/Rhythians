...
export async function syncRhythiaMaps(status?: RhythiaMapStatus) {
  ...
  for (const currentStatus of statuses) {
    ...
    for (const rawMap of maps) {
      ...
      else if (isRanked) { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "approved", reviewerNote: null, reviewedById: importer.id, reviewedAt: new Date() } }); await prisma.$executeRawUnsafe(`UPDATE "MapDifficultyAnalysis" SET "sourceStatus"='ranked',"pointEligible"=CASE WHEN status='analyzed' AND "analyzerVersion"=$2 THEN TRUE ELSE "pointEligible" END,"updatedAt"=CURRENT_TIMESTAMP WHERE "mapId"=$1`, existing.id, MAP_ANALYZER_VERSION); results.promoted += 1; }
