...
export async function syncRhythiaMaps(status?: RhythiaMapStatus) {
  ...
  for (const currentStatus of statuses) {
    ...
    for (const map of maps) {
      ...
      else if (isRanked) { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "approved", reviewerNote: null, reviewedById: importer.id, reviewedAt: new Date() } }); await prisma.$executeRawUnsafe(`UPDATE "MapDifficultyAnalysis" SET "sourceStatus"='ranked',"pointEligible"=CASE WHEN status='analyzed' THEN TRUE ELSE "pointEligible" END,"updatedAt"=CURRENT_TIMESTAMP WHERE "mapId"=$1`, existing.id); results.promoted += 1; }
      else { await prisma.challengeMap.update({ where: { id: existing.id }, data: { ...updateData, status: "approved", reviewerNote: UNRANKED_MAP_MARKER, reviewedById: importer.id, reviewedAt: new Date() } }); await prisma.$executeRawUnsafe(`UPDATE "MapDifficultyAnalysis" SET "sourceStatus"='unranked',"pointEligible"=FALSE,"updatedAt"=CURRENT_TIMESTAMP WHERE "mapId"=$1`, existing.id); results.updated += 1; }
    }
  }
