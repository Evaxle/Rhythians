...
export async function getCachedModePoints(userId: string): Promise<ReliableModePoints> {
  ...
  return { points: { lock: overrides.get("rpl") ?? baseline.rplBase + raw.lock, spin: overrides.get("rps") ?? baseline.rpsBase + raw.spin, vr: overrides.get("rpv") ?? baseline.rpvBase + raw.vr }, rhp: overrides.get("rhp") ?? user?.rhp ?? 0, source: "cached", syncedAt: user?.lastRhythiaRpCheckAt ?? null, warning: null };
}
