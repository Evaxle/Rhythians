...
export async function selectBattleMap(rankIndex: number) {
  ...
  const maps = await prisma.$queryRawUnsafe<any[]>(`SELECT id,title,artist,rating,length,"mapFileUrl","imageUrl" FROM "ChallengeMap" WHERE status='approved' AND rating >= $1 AND rating <= $2 ORDER BY "updatedAt" DESC LIMIT 50`, rank.rangeMin, rank.rangeMax);
