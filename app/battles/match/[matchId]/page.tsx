...
import { BattleReadyPanel } from "@/components/battles/battle-ready-panel";
...
export default async function BattleMatchPage({ params }: { params: Promise<{ matchId: string }> }) {
  ...
  if (!user) return null;
  return <div className="space-y-5"><BattleMatchApp matchId={matchId} /><BattleReadyPanel matchId={matchId} /><RbpBattleOverlay matchId={matchId} /><BattleChat matchId={matchId} currentUserId={user.id} /></div>;
}
