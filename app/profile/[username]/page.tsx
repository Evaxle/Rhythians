...
function loadProfileUser(username: string) {
  return prisma.user.findFirst({ where: { profileHandle: username }, include: { clips: { where: { status: "approved" }, orderBy: { createdAt: "desc" }, include: { category: true, reviewedBy: { select: { username: true, displayName: true } } } }, articleRevisions: true, roles: { include: { role: true } }, playerRank: true, userTags: { include: { tag: true } }, rhythiaProfile: true } });
}
...
export default async function ProfilePage({ params }: Props) {
  ...
  try { user = await loadProfileUser(username); } catch { const bare = await prisma.user.findFirst({ where: { profileHandle: username }, include: { roles: { include: { role: true } } } }); if (bare) user = { ...bare, clips: [], articleRevisions: [], playerRank: null, userTags: [], rhythiaProfile: null } as Awaited<ReturnType<typeof loadProfileUser>>; }
  ...
  const selectedTags = selectedTagIds.length > 0 ? selectedTagIds.map((id) => user.userTags.find((entry) => entry.tagId === id)).filter((entry): entry is NonNullable<typeof entry> => Boolean(entry)) : user.userTags.slice(0, 3);
  const editableTags = user.userTags.filter((entry) => entry.tag.slug !== "rhythian-coach");
  const editableSelectedTagIds = selectedTags.filter((entry) => entry.tag.slug !== "rhythian-coach").map((entry) => entry.tagId);
  ...
  if (presence?.lastActiveAt) { const seconds = Math.floor((Date.now() - presence.lastActiveAt.getTime()) / 1000); if (seconds < 60) presenceLabel = "Active now"; else if (seconds < 3600) presenceLabel = `Active ${Math.floor(seconds / 60)}m ago`; else if (seconds < 86400) presenceLabel = `Active ${Math.floor(seconds / 3600)}h ago`; else if (seconds < 2592000) presenceLabel = `Active ${Math.floor(seconds / 86400)}d ago`; else presenceLabel = `Last seen ${presence.lastActiveAt.toLocaleDateString()}`; } else presenceLabel = "Never seen online";
  return (
    <div className="space-y-8">
      <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow"><div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between"><div className="flex items-start gap-6">{avatarUrl ? <img src={avatarUrl} alt={`${user.username}'s avatar`} className="h-24 w-24 rounded-full border-2 border-accent/30" /> : <div className="flex h-24 w-24 items-center justify-center rounded-full border-2 border-accent/30 bg-accent/10 text-3xl font-bold text-accent">{user.username.charAt(0).toUpperCase()}</div>}<div><p className="text-sm uppercase tracking-[0.3em] text-accent">Profile</p><h1 className="mt-2 flex flex-wrap items-center gap-3 text-3xl font-semibold text-white"><UserName username={user.displayName ?? user.username} isCoach={isCoach} />{user.rhythiaProfile?.flag && <FlagIcon flag={user.rhythiaProfile.flag} country={user.rhythiaProfile.country} size="md" />}{user.rhythiaVerified && <RhythiaVerifiedBadge size="sm" />}{user.rhythiaProfile && presence && <span title={presenceLabel} className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium ${presence.isOnline ? "border-emerald-400/40 bg-emerald-400/10 text-emerald-300" : "border-red-400/40 bg-red-400/10 text-red-300"}`}><span className={`h-2 w-2 rounded-full ${presence.isOnline ? "bg-emerald-400" : "bg-red-400/70"}`} />{presence.isOnline ? "Online" : "Offline"}</span>}</h1>{profileTitle && <p className="mt-1 text-base font-semibold" style={{ color: profileTitle.color }}>{profileTitle.title}</p>}<p className="mt-1 text-sm text-muted">@{user.profileHandle}</p>{selectedTags.length > 0 && <div className="mt-4"><UserTags tags={selectedTags} size="md" /></div>}{user.bio && <p className="mt-4 text-sm leading-7 text-muted">{user.bio}</p>}</div></div><div className="flex flex-col items-stretch gap-3">{!isOwnProfile && <FriendButton userId={user.id} />}{isOwnProfile && <div className="flex items-center gap-2"><div className="flex-1"><RhythiaConnect connectedUrl={user.rhythiaProfile?.profileUrl} /></div>{user.rhythiaProfile && <ProfileScoreRefresh />}</div>}{!isOwnProfile && <Link href={`/messages?user=${encodeURIComponent(user.profileHandle)}`} className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition hover:border-accent/40"><MessageCircle size={16} /> Message</Link>}{currentUser && !isOwnProfile && <ReportButton targetType="user" targetId={user.id} targetLabel={user.username} />}<div className="rounded-3xl border border-border bg-background/80 px-5 py-4 text-sm text-muted"><p>Member since</p><p className="mt-2 text-lg font-semibold text-white">{user.joinedAt.toLocaleDateString()}</p></div>{user.avgMapRating != null && <div className="rounded-3xl border border-border bg-background/80 px-5 py-4 text-sm text-muted"><p>Average map rating</p><p className="mt-2 text-lg font-semibold text-white">{user.avgMapRating.toFixed(2)}</p></div>}<div className="rounded-3xl border border-accent/30 bg-accent/10 px-5 py-4"><RankProgress rhp={user.rhp} globalRank={globalRank} highestSkillLevel={challengeLevel} /></div></div></div></section>
