...
export default async function ProfilePage({ params }: Props) {
  ...
  const [rankResult, categoryResult, challengeResult, titleResult] = await Promise.allSettled([getUserGlobalRank(user.id), getUserCategoryLevels(user.id), getUserChallengeLevel(user.id), prisma.$queryRawUnsafe<Array<{ title: string; color: string }>>('SELECT "title", "color" FROM "UserProfileTitle" WHERE "userId" = $1 LIMIT 1', user.id)]);
