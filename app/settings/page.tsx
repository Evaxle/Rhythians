    ...
    <div className="space-y-8">
      ...
      {fullUser.rhythiaProfile && (
        <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
          ...
          <p className="mt-2 text-sm leading-7 text-muted">Your past Rhythia scores are imported automatically when you link your account. Only maps inside your current rank&apos;s rating range award RHP. If you&apos;ve played more ranked maps since then, run the scan again to claim the RHP for any new completions.</p>
          <div className="mt-6 border-t border-border pt-6"><CheckAllScoresButton /></div>
        </section>
      )}
      ...
      <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
        ...
        <p className="mt-2 text-sm leading-7 text-muted">{fullUser.discordId ? "Your tags are synced from your Discord roles. You can also adjust your onboarding answers below." : "Change what you answered during onboarding — your tags update automatically."}</p>
      ...
      <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
        ...
        <p className="mt-2 text-sm leading-7 text-muted">Toggle the Rhythia-style custom cursor and its trail.</p>
        <div className="mt-6 border-t border-border pt-6"><CursorSettings /></div>
      </section>
      ...
      <section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white">{fullUser.username}#{fullUser.discriminator}</h2>
            <p className="mt-1 text-sm text-muted">{fullUser.inGuild ? <span className="text-green-300">Connected to {guild?.name ?? "the server"}</span> : "Not in the Discord server"}</p>
          </div>
          <DiscordSyncButton />
        </div>
        {roleNames.length > 0 && <div className="mt-8"><p className="mb-3 text-sm uppercase tracking-[0.24em] text-accent">Discord roles</p><div className="flex flex-wrap gap-1.5">{roleNames.map((roleName) => <span key={roleName} className="inline-flex items-center rounded-full border border-border bg-white/5 px-2.5 py-1 text-xs text-white">{roleName}</span>)}</div></div>}
        <div className="mt-8 border-t border-border pt-6"><Link href={`https://discord.com/channels/${guildId ?? ""}`} className="text-sm font-semibold text-accent transition hover:text-accent/80">Open the Discord server →</Link></div>
      </section>
    </div>
  );
}
