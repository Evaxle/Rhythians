    ...
    <section className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {announcements.length > 0 ? (
          ...
          <p className="rounded-3xl border border-border bg-background/80 p-5 text-sm text-muted md:col-span-3">
    ...
    <div className="space-y-10">
          ...
          <div className="rounded-3xl border border-border bg-background/80 p-6 text-sm text-muted shadow-inner">
            <div className="rounded-3xl border border-white/5 bg-gradient-to-b from-white/5 to-transparent p-6">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl border border-border bg-surface p-5"><p className="text-sm text-muted">Members</p><p className="mt-2 text-3xl font-semibold">{stats.members}</p></div>
                <div className="rounded-3xl border border-border bg-surface p-5"><p className="text-sm text-muted">Online</p><p className="mt-2 text-3xl font-semibold">{stats.online}</p></div>
                <div className="rounded-3xl border border-border bg-surface p-5"><p className="text-sm text-muted">Articles</p><p className="mt-2 text-3xl font-semibold">{stats.articles}</p></div>
                <div className="rounded-3xl border border-border bg-surface p-5"><p className="text-sm text-muted">Clips</p><p className="mt-2 text-3xl font-semibold">{stats.clips}</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <article className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent/15 text-accent"><BookOpen size={24} /></div>
          ...
          <Link href="/wiki" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent transition hover:text-white">View articles <ArrowRight size={16} /></Link>
        </article>
        <article className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent/15 text-accent"><Video size={24} /></div>
          ...
          <Link href="/clips" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent transition hover:text-white">Browse clips <ArrowRight size={16} /></Link>
        </article>
        <article className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent/15 text-accent"><Sparkles size={24} /></div>
          ...
          <Link href="/community" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-accent transition hover:text-white">See community <ArrowRight size={16} /></Link>
        </article>
      </section>

      <section className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
        <div className="flex items-center gap-3 text-sm uppercase tracking-[0.24em] text-accent"><Sparkles /> Featured Clips</div>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {featuredClips.length > 0 ? featuredClips.map((clip) => (
            <Link key={clip.id} href={`/clips/${clip.id}`} className="group overflow-hidden rounded-3xl border border-border bg-background/90 transition hover:-translate-y-0.5 hover:shadow-glow">
              <div className="aspect-video bg-white/5" />
              <div className="p-5"><p className="text-xs uppercase tracking-[0.24em] text-accent">{clip.category?.name ?? "Clip"}</p><h3 className="mt-2 text-lg font-semibold text-white">{clip.title}</h3><p className="mt-2 text-sm text-muted">By {clip.uploader.username}</p></div>
            </Link>
          )) : <p className="rounded-3xl border border-border bg-background/90 p-6 text-sm text-muted">No featured clips have been selected yet.</p>}
        </div>
      </section>
    </div>
  );
