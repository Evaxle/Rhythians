...
export default function AdminSettingsPage() {
  ...
  async function publish() {
    ...
    try {
      ...
      const [settingsUpload, videoUpload] = await Promise.all([fetch(uploadData.settingsUploadUrl, { method: "PUT", headers: { "Content-Type": settingsFile.type || "application/octet-stream" }, body: settingsFile }), fetch(uploadData.videoUploadUrl, { method: "PUT", headers: { "Content-Type": videoFile.type || "video/mp4" }, body: videoFile })]);
      if (!settingsUpload.ok || !videoUpload.ok) throw new Error("One of the files failed to upload.");
