...
export default async function AdminChallengePage() {
  ...
  return <div className="space-y-6"><section className="rounded-3xl border border-border bg-surface/95 p-8 shadow-glow"><p className="text-sm uppercase tracking-[0.3em] text-accent">Challenge</p><h1 className="mt-2 text-3xl font-semibold text-white">Challenge level management</h1><p className="mt-3 text-sm leading-7 text-muted">Assign approved Challenge maps to Levels 1-10. Maps that are not assigned to a level do not appear in Challenge progression.</p></section><ChallengeMapLevelManager initialMaps={maps.map((map) => ({ ...map, level: levels.get(map.id) ?? null }))} /></div>;
}
