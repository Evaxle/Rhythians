...
import { RhythianPathUserActions } from "@/components/admin/rhythian-path-user-actions";

export const dynamic = "force-dynamic";

function formatLength(length: number | null | undefined) {
  if (length == null || !Number.isFinite(length)) return "—";
  const totalSeconds = Math.max(0, Math.round(length / 1000));
  return `${Math.floor(totalSeconds / 60)}:${String(totalSeconds % 60).padStart(2, "0")}`;
}

function pathMinimumRating(index: number) {
  if (index === RANKS.length - 1) return 3.9;
  return Math.max(0, Math.round((RANKS[index + 1].rangeMin - 0.1) * 100) / 100);
}
        ...
        {path.ranks.map((rank) => {
          ...
          return (
            <article key={rank.name} className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
              ...
              <div className="mt-5"><p className="text-xs uppercase tracking-[0.2em] text-accent">Users at this path rank</p>{users.length === 0 ? <p className="mt-3 text-sm text-muted">No users have completed through this rank.</p> : <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">{users.map((user) => <div key={user.userId} className="flex items-center justify-between rounded-2xl border border-border bg-background/60 px-4 py-3"><Link href={`/profile/${user.profileHandle}`} className="min-w-0 flex-1 truncate font-semibold text-white transition hover:text-accent"><span className="truncate">{user.displayName ?? user.username}</span><span className="ml-3 text-xs text-muted">@{user.profileHandle}</span></Link><RhythianPathUserActions userId={user.userId} pathRank={user.maxRank ?? -1} /></div>)}</div>}</div>
            </article>
          );
        })}
      </section>
