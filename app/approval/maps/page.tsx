...
export default async function ApprovalMapsPage() {
  ...
  const loaded = await getPendingChallengeMaps({ take: 6 });
  const maps = loaded.slice(0, 5);
  const hasMore = loaded.length > 5;
  ...
  const nextCursor = hasMore && last ? { createdAt: last.createdAt.toISOString(), id: last.id } : null;

  return <div className="ui-page space-y-6"><section className="ui-page-header"><p className="ui-eyebrow">Map review team</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">Review pending map submissions</h1><p className="mt-3 text-sm leading-7 text-muted">Ranked submissions join the ranked map pool and use RHP. Challenge submissions are placed into the Main Challenge or a skill category at the selected level and never award RHP.</p></section><MapReviewQueueV2 initialMaps={pending} initialHasMore={hasMore} initialCursor={nextCursor} /></div>;
}
