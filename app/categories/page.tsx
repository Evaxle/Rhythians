...
export default async function CategoriesPage({ searchParams }: Props) {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <section className="rounded-3xl border border-border bg-surface/95 p-10 text-center shadow-glow">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted">Skill categories and Challenge progression are reserved for members with a linked Rhythia account. Sign in to get started.</p>
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <section className="rounded-3xl border border-border bg-surface/95 p-10 text-center shadow-glow">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted">To play category and Challenge maps you first need to link your Rhythia profile.</p>
  ...
  let challengeMaps: Awaited<ReturnType<typeof getChallengeMapsWithCompletions>>;
  let challengeLevel: number;

  try {
    [levels, stats, mapsByCategory, challengeMaps, challengeLevel] = await Promise.all([
      ...
      getChallengeMapsWithCompletions(user.id),
      getUserChallengeLevel(user.id),
    ]);
  } catch {
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <ErrorState title="Categories unavailable" description="The categories or Challenge database tables are missing or not migrated yet. Run npm run db:migrate (or ask an admin to run the database repair endpoint) and try again." />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <section className="space-y-2">
        <p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><Layers size={16} /> Categories</p>
        <h1 className="text-3xl font-semibold text-white">Skill categories</h1>
        <p className="max-w-2xl text-sm leading-7 text-muted">Climb levels 1-10 in Jumps, Stream, Tech, and Off Grid. Challenge is now a tab here with 20 manually curated levels.</p>
      </section>

      <CategoriesBrowser levels={levels} stats={stats} maps={maps} currentUserId={user.id} challengeMaps={challengeMaps} challengeLevel={challengeLevel} defaultTab={defaultTab as "maps" | "leaderboards" | "stats" | "challenge"} />
    </div>
  );
}
