...
export default async function CategoriesPage({ searchParams }: Props) {
  ...
  return <div className="mx-auto max-w-5xl space-y-8"><section className="space-y-2"><p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><Layers size={16} /> Categories</p><h1 className="text-3xl font-semibold text-white">Skill categories</h1><p className="max-w-2xl text-sm leading-7 text-muted">Climb levels 1-10 in Challenge, Jumps, Stream, Tech, Off Grid, and Vibro from one maps browser.</p></section><CategoriesBrowser levels={levels} stats={stats} maps={maps} currentUserId={user.id} challengeMaps={challengeMaps} challengeLevel={challengeLevel} isAdmin={isAdmin} defaultTab={defaultTab as "maps" | "leaderboards" | "stats" | "challenge"} /></div>;
}
