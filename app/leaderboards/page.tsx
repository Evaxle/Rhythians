...
export default async function LeaderboardsPage() {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <section className="rounded-3xl border border-border bg-surface/95 p-10 text-center shadow-glow">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted">Leaderboards are only available to members who link their Rhythia account. Sign in to get started.</p>
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <section className="rounded-3xl border border-border bg-surface/95 p-10 text-center shadow-glow">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted">To access the leaderboards you first need to link your Rhythia profile.</p>
  ...
  const challengeLeaderboards = await Promise.all([0, 1, 2, 3, 4, 5, 6, 7, 8].map((rankIndex) => getChallengeLeaderboard(rankIndex, 100)));
  const challengeLevelLeaderboard = await getChallengeLevelLeaderboard(100);

  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <section className="space-y-2">
        <p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><Trophy size={16} /> Leaderboards</p>
        <h1 className="text-3xl font-semibold text-white">Rhythian standings</h1>
        <p className="text-sm text-muted">Compare daily results, ranked map RHP, and the separate Challenge Level progression.</p>
      </section>
      <LeaderboardTabs dailyLeaderboards={dailyLeaderboards} challengeLeaderboards={challengeLeaderboards} challengeLevelLeaderboard={challengeLevelLeaderboard} currentUserId={user.id} />
    </div>
  );
}
