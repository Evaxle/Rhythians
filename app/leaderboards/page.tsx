...
import { reconcileAllStoredRhp } from "@/lib/rhp-reconcile";
...
export default async function LeaderboardsPage() {
  ...
  await syncUserModeScores(user.id).catch(() => null);
  await reconcileAllStoredRhp().catch(() => null);
  const freshUser = await prisma.user.findUnique({ where: { id: user.id }, select: { rhp: true } });
  const freshRhp = freshUser?.rhp ?? user.rhp;
  const freshRank = getRankInfo(freshRhp);
  const userRank = freshRank.index;
  ...
  const modeLeaderboards = {
    lock: await getModeLeaderboard("lock", 100),
    spin: await getModeLeaderboard("spin", 100),
    vr: await getModeLeaderboard("vr", 100),
  };

  return <div className="ui-page space-y-6"><section className="ui-page-header"><div className="relative flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><Trophy size={16} /> Leaderboards</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.04em] text-white">Rhythian standings</h1><p className="mt-2 max-w-3xl text-sm leading-7 text-muted">Compare RHP global standings, rank bands, separate Lock/Spin/VR point ladders, and Challenge Level progression.</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3"><p className="text-xs uppercase tracking-[0.18em] text-muted">Your rank</p><p className="mt-1 text-sm font-semibold text-white">{freshRank.isExpert ? "Expert" : `${freshRank.name} ${freshRank.tier}`}</p></div><div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[0.04] px-4 py-3"><p className="text-xs uppercase tracking-[0.18em] text-muted">RHP</p><p className="mt-1 text-sm font-semibold text-white">{freshRhp.toLocaleString()}</p></div><div className="hidden rounded-2xl border border-violet-400/15 bg-violet-400/[0.04] px-4 py-3 sm:block"><p className="text-xs uppercase tracking-[0.18em] text-muted">Mode ladders</p><p className="mt-1 text-sm font-semibold text-white">RPL · RPS · RPV</p></div></div></div></section><section className="ui-card rounded-[2rem] p-5 sm:p-7"><LeaderboardTabs dailyLeaderboards={dailyLeaderboards} challengeLeaderboards={rankedLeaderboards} modeLeaderboards={modeLeaderboards} challengeLevelLeaderboard={challengeLevelLeaderboard} currentUserId={user.id} initialRankIndex={userRank} /></section></div>;
}
