...
export default async function LeaderboardsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-8"><section className="space-y-2"><p className="flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-accent"><Trophy size={16} /> Leaderboards</p><h1 className="text-3xl font-semibold text-white">Rhythian standings</h1><p className="text-sm text-muted">Compare daily results, ranked map RHP, and the separate Challenge Level progression.</p></section><LeaderboardTabs dailyLeaderboards={dailyLeaderboards} challengeLeaderboards={challengeLeaderboards} challengeLevelLeaderboard={challengeLevelLeaderboard} currentUserId={user.id} /></div>;
}
