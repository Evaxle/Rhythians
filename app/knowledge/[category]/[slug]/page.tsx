...
export default async function KnowledgeArticlePage({ params }: Props) {
  ...
  if (!article || article.category.slug !== category || !article.published) {
    return notFound();
  }
        ...
        <aside className="space-y-6">
          <div className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
            ...
            <p className="mt-3 text-base font-semibold text-white">
              {article.author.username}#{article.author.discriminator}
            </p>
            <p className="mt-2 text-sm text-muted">
              {article.author.roles.length > 0
                ? article.author.roles
            ...
            <div className="rounded-3xl border border-border bg-surface/95 p-6 shadow-glow">
              ...
              <div className="mt-4 space-y-3 text-sm text-muted">
                {toc.map((item) => (
                  ...
                  >
                    <span className={item.level === 3 ? "pl-4" : ""}>
                      {item.text}
                    </span>
                  </a>
                ))}
              ...
              <div className="mt-4 space-y-3">
                {related.map(
                  (item: {
                    ...
                    <Link
                      key={item.id}
                      href={`/knowledge/${item.category.slug}/${item.slug}`}
