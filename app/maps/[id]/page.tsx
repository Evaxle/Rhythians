...
export default async function RankedMapPage({ params }: Props) {
  ...
  return <div className="mx-auto max-w-6xl space-y-4">
    {map.isLegacy ? <div className="rounded-2xl border border-white/15 bg-white/[0.045] p-4 text-sm text-white"><div className="flex flex-wrap items-center justify-between gap-3"><span className="flex items-center gap-2"><Archive size={16} className="text-accent" /> Legacy Rhythia map — analyzed for difficulty, timeline, and rankability reference only.</span><CopyMapId mapId={map.mapId} /></div></div> : map.isRanked ? <div className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-4 text-sm text-emerald-200"><div className="flex flex-wrap items-center justify-between gap-3"><span>Ranked map — current analysis provides its difficulty, rewards, timeline, and rankability.</span><CopyMapId mapId={map.mapId} /></div></div> : <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-200"><div className="flex flex-wrap items-center justify-between gap-3"><span>Unranked map — analyzed for difficulty, timeline, and rankability. Rankability does not automatically change this map&apos;s status.</span><CopyMapId mapId={map.mapId} /></div></div>}
