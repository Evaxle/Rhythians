...
import { getVideoUrl } from "@/lib/clips";
...
export default async function ClipPage({ params }: Props) {
  ...
  const clip = await prisma.clip.findUnique({
    ...
    include: {
      ...
      comments: {
        ...
        include: { author: { include: { userTags: { include: { tag: true } }, playerRank: true, rhythiaProfile: { select: { country: true, flag: true } } } } },
      },
      coachComments: {
        orderBy: { createdAt: "desc" },
        include: { author: { include: { userTags: { include: { tag: true } }, playerRank: true, rhythiaProfile: { select: { country: true, flag: true } } } } },
  ...
  const canViewClip = clip.status === "approved" || (Boolean(sessionUser) && (sessionUser!.id === clip.uploaderId || clip.reviewedById === sessionUser!.id || (await canAccessAdmin(sessionUser))));
  if (!canViewClip) return notFound();

  const videoUrl = await getVideoUrl(clip.storagePath);
  let uploaderPresence: { isOnline: boolean; lastActiveAt: Date | null } | null = null;
  if (clip.uploader.rhythiaProfile) uploaderPresence = await getRhythiaStatus(clip.uploader.rhythiaProfile);

  let isLiked = false;
  let isCoach = false;
  if (sessionUser) {
    isLiked = clip.likes.some((like) => like.userId === sessionUser.id);
    const user = await prisma.user.findUnique({ where: { id: sessionUser.id }, include: { userTags: { include: { tag: true } } } });
      ...
      <section className="ui-page-header">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <div className="mt-2 flex items-center gap-3"><CopyClipId clipId={clip.id} /><p className="text-xs text-muted">Tag it with <span className="font-semibold text-white">/clip {clip.id.slice(0, 8)}…</span></p></div>
            <div className="mt-3 flex items-center gap-3">
              {getAvatarUrl(clip.uploader, 64) ? <img src={getAvatarUrl(clip.uploader, 64)!} alt="" className="h-8 w-8 rounded-full" /> : <div className="flex h-8 w-8 items-center justify-center rounded-full bg-accent/20 text-xs font-semibold text-accent">{clip.uploader.username.charAt(0).toUpperCase()}</div>}
              <div>
                <p className="flex items-center gap-2 text-sm font-medium text-white">
                  {clip.uploader.username}#{clip.uploader.discriminator}
                  {clip.uploader.rhythiaProfile && <FlagIcon flag={clip.uploader.rhythiaProfile.flag} country={clip.uploader.rhythiaProfile.country} />}
                  {uploaderPresence && <span title={uploaderPresence.isOnline ? "Online" : "Offline"} className={`inline-block h-2 w-2 rounded-full ${uploaderPresence.isOnline ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]" : "bg-red-400/70"}`} />}
                </p>
                ...
                <p className="text-xs text-muted">{clip.createdAt.toLocaleDateString()}</p>
              </div>
          ...
          <div className="grid gap-3 text-sm text-muted">
            <LikeButton clipId={clip.id} initialLikes={clip.likes.length} isLiked={isLiked} isAuthenticated={Boolean(sessionUser)} />
            <div className="rounded-3xl border border-border bg-background/80 p-4"><p className="text-xs uppercase tracking-[0.24em] text-accent">Comments</p><p className="mt-2 text-2xl font-semibold text-white">{clip.comments.length}</p></div>
