...
export async function POST() {
  ...
  let challengeChecked = 0, challengePasses = 0;
  if (challengeTarget != null) {
    ...
    for (const map of maps) {
      ...
      await prisma.challengeMapCompletion.upsert({ where: { challengeMapId_userId: { challengeMapId: map.id, userId: user.id } }, create: { challengeMapId: map.id, userId: user.id, rating: map.rating ?? 0, accuracy, passed: true, points: 0, scoreId: hit.id }, update: { rating: map.rating ?? 0, accuracy, passed: true, points: 0, scoreId: hit.id } });
      challengePasses += 1;
    }
  }
