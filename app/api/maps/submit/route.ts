...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) {
    return NextResponse.json(
      { error: "You've submitted too many maps recently. Please try again later." },
  ...
  if (!Number.isFinite(rating) || rating < 0 || rating > 9.99) {
    return NextResponse.json({ error: "Requested rating must be between 0 and 9.99." }, { status: 400 });
  }

  // A Rhythia URL can be provided instead of (or in addition to) a file upload.
  ...
  if (parsedMapUrl) {
    ...
    if (!fetched) {
      return NextResponse.json({ error: "That Rhythia map could not be found." }, { status: 404 });
    }
  ...
  if (!resolvedTitle.trim() || resolvedTitle.length > 120) {
    return NextResponse.json({ error: "Title is required and must be under 120 characters." }, { status: 400 });
  }
  if (!resolvedMapFile) {
    return NextResponse.json({ error: "Provide a map file or a valid Rhythia map URL." }, { status: 400 });
  }
  ...
  try {
    ...
    if (error?.code === "P2002") {
      return NextResponse.json(
        { error: "This Rhythia map is already on the site. Browse it in the Maps section instead." },
        { status: 409 }
      );
    }
  ...
  await prisma.moderationAction.create({
    data: {
      actorId: user.id,
      action: "map_submitted",
      targetType: "challenge_map",
      targetId: map.id,
      metadata: { title: map.title, requestedRating: map.requestedRating, sourceUrl: sourceUrl },
    },
  });

  return NextResponse.json({ mapId: map.id, status: map.status });
}