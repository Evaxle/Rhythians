...
async function buildDownloadStream(maps: Array<{ id: string; title: string; mapFileUrl: string }>, withProgress: boolean) {
  ...
  let offset = 0; let downloaded = 0; let processed = 0; const central: Buffer[] = []; const failures: string[] = [];
  const output = new ReadableStream<Uint8Array>({
    start(controller) {
      void (async () => {
        const sendProgress = () => { if (withProgress) controller.enqueue(frame(1, Buffer.from(JSON.stringify({ downloaded, processed, total: maps.length }), "utf8"))); };
        sendProgress();
        for (const map of maps) {
          try {
            const data = await fetchMap(map.mapFileUrl, `https://www.rhythia.com/maps/${map.id}`);
            downloaded += 1;
            const name = `${String(downloaded).padStart(3, "0")} - ${safeName(map.title)}${extensionFromUrl(map.mapFileUrl)}`;
            const checksum = crc32(data);
            const local = localRecord(name, data, time, day, checksum);
            const centralEntry = centralRecord(name, data, time, day, checksum, offset);
            if (withProgress) controller.enqueue(frame(2, local)); else controller.enqueue(local);
            central.push(centralEntry); offset += local.length;
          } catch (error) { failures.push(`${map.title}: ${error instanceof Error ? error.message : "download failed"}`); }
          processed += 1; sendProgress();
        }
        if (failures.length > 0) {
          const data = Buffer.from(`${failures.join("\n")}\n`, "utf8"); const name = "download-errors.txt"; const checksum = crc32(data); const local = localRecord(name, data, time, day, checksum); central.push(centralRecord(name, data, time, day, checksum, offset)); if (withProgress) controller.enqueue(frame(2, local)); else controller.enqueue(local); offset += local.length;
        }
        const centralData = Buffer.concat(central); const end = endRecord(central.length, centralData.length, offset);
        if (withProgress) { controller.enqueue(frame(2, centralData)); controller.enqueue(frame(2, end)); } else { controller.enqueue(centralData); controller.enqueue(end); }
        controller.close();
      })().catch((error) => { if (withProgress) controller.enqueue(frame(3, Buffer.from(JSON.stringify({ message: error instanceof Error ? error.message : "Could not build the ZIP." }), "utf8"))); controller.close(); });
    },
  });
  return output;
}
...
export async function GET(request: Request) {
  ...
  const data = await getApprovedMaps(true, user.id); if (!data.rankInfo) return NextResponse.json({ error: "Unable to determine your rank." }, { status: 400 });
  const rankInfo = getRankInfo(user.rhp); const maps = data.maps.filter((map) => { const inRank = map.rating != null && map.rating >= rankInfo.rangeMin && map.rating <= rankInfo.rangeMax; if (!inRank) return false; if (mode === "legacy") return map.isLegacy; if (mode === "ranked-legacy") return map.isRanked || map.isLegacy; return map.isRanked; }).map((map) => ({ id: map.id, title: map.title, mapFileUrl: map.mapFileUrl }));
