...
function extensionFromUrl(url: string) { try { const pathname = new URL(url).pathname; return pathname.match(/(\.[a-z0-9]{2,8})$/i)?.[1] ?? ".sspm"; } catch { return ".sspm"; } }
...
function buildZip(files: Array<{ name: string; data: Buffer }>) { const localParts: Buffer[] = []; const centralParts: Buffer[] = []; let offset = 0; const { time, day } = dosTimeAndDate(); for (const file of files) { const name = Buffer.from(file.name, "utf8"); const size = file.data.length; const checksum = crc32(file.data); const local = Buffer.concat([u32(0x04034b50), u16(20), u16(0x0800), u16(0), u16(time), u16(day), u32(checksum), u32(size), u32(size), u16(name.length), u16(0), name, file.data]); localParts.push(local); centralParts.push(Buffer.concat([u32(0x02014b50), u16(20), u16(20), u16(0x0800), u16(0), u16(time), u16(day), u32(checksum), u32(size), u32(size), u16(name.length), u16(0), u16(0), u16(0), u16(0), u32(0), u32(offset), name])); offset += local.length; } const local = Buffer.concat(localParts); const central = Buffer.concat(centralParts); const end = Buffer.concat([u32(0x06054b50), u16(0), u16(0), u16(files.length), u16(files.length), u32(central.length), u32(local.length), u16(0)]); return Buffer.concat([local, central, end]); }
...
export async function GET(request: Request) {
  ...
  const withProgress = params.get("progress") === "1";
  ...
  const maps = data.maps.filter((map) => { const inRank = map.rating != null && map.rating >= rankInfo.rangeMin && map.rating <= rankInfo.rangeMax; if (!inRank) return false; if (mode === "legacy") return map.isLegacy; if (mode === "ranked-legacy") return map.isRanked || map.isLegacy; return map.isRanked; });
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      void (async () => {
        ...
        let downloaded = 0;
        const queue = [...maps];
        const sendProgress = () => controller.enqueue(encoder.encode(`PROGRESS ${JSON.stringify({ downloaded, processed, total: maps.length })}\n`));
        controller.enqueue(encoder.encode(`PROGRESS ${JSON.stringify({ downloaded: 0, processed: 0, total: maps.length })}\n`));
        const workers = Array.from({ length: Math.min(8, queue.length) }, async () => { while (queue.length > 0) { const map = queue.shift(); if (!map) return; try { const fileData = await fetchMap(map.mapFileUrl, `https://www.rhythia.com/maps/${map.id}`); downloaded += 1; files.push({ name: `${String(downloaded).padStart(3, "0")} - ${safeName(map.title)}${extensionFromUrl(map.mapFileUrl)}`, data: fileData }); } catch (error) { failures.push(`${map.title}: ${error instanceof Error ? error.message : "download failed"}`); } finally { processed += 1; sendProgress(); } } });
        await Promise.all(workers);
        if (files.length === 0) { controller.enqueue(encoder.encode(`ERROR ${JSON.stringify({ message: `Rhythia's map files could not be downloaded. ${failures.slice(0, 3).join(" | ")}`, failed: failures.length, total: maps.length })}\n`)); controller.close(); return; }
        ...
        controller.close();
      })().catch((error) => { controller.enqueue(encoder.encode(`ERROR ${JSON.stringify({ message: error instanceof Error ? error.message : "Could not build the ZIP." })}\n`)); controller.close(); });
    },
  });
  ...
  if (withProgress) return new Response(stream, { headers: { "Content-Type": "application/x-rhythians-map-download", "Cache-Control": "no-store", "X-Map-Total": String(maps.length), "Content-Disposition": `attachment; filename="${safeName(`${rankName}-${suffix}-maps`)}.zip"` } });
  const chunks: Uint8Array[] = []; const reader = stream.getReader(); while (true) { const result = await reader.read(); if (result.done) break; chunks.push(result.value); }
  const output = Buffer.concat(chunks.map((chunk) => Buffer.from(chunk)));
  const marker = Buffer.from("\nRHYM_ZIP_START\n"); const markerIndex = output.indexOf(marker); if (markerIndex < 0) return NextResponse.json({ error: "Could not build the ZIP." }, { status: 502 });
  const zip = output.subarray(markerIndex + marker.length);
