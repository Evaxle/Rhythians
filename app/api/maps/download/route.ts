...
function extensionFromPath(path: string) {
  return path.split("?")[0].split("#")[0].split(".").pop()?.toLowerCase() === "rhm" ? ".rhm" : ".sspm";
}

async function resolveMapSource(value: string) {
  if (/^https?:\/\//i.test(value)) {
    return { url: value, extension: extensionFromMapUrl(value), originalName: fileNameFromUrl(value) };
  }
...
export async function GET(request: Request) {
  ...
  const map = await prisma.challengeMap.findUnique({
    where: { id },
    select: { mapFileUrl: true, status: true, title: true, isAutoImported: true },
  });
