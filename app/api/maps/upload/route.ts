...
const VALID_MAP_EXTENSIONS = ["sspm", "rhm", "osu", "zip"];
...
export async function POST(request: Request) {
  ...
  const { fileName, contentType, folder, fileSize } = body as {
    fileName?: string;
    contentType?: string;
    folder?: string;
    fileSize?: number;
  ...
  if (folder === "maps") {
    if (!VALID_MAP_EXTENSIONS.includes(extension)) {
      return NextResponse.json({ error: "Map file must be .sspm, .rhm, .osu, or .zip." }, { status: 400 });
    }
    if (fileSize && fileSize > 50 * 1024 * 1024) {
      return NextResponse.json({ error: "Map file must be 50 MB or smaller." }, { status: 400 });
    }
  } else if (folder === "map-images") {
    if (!VALID_IMAGE_TYPES.includes(contentType)) {
      return NextResponse.json({ error: "Image must be PNG, JPG, JPEG, or WEBP." }, { status: 400 });
    }
  } else {
