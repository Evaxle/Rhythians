...
export async function PATCH(request: Request, { params }: Props) {
  ...
  if (body.rhp !== undefined || body.rpl !== undefined || body.rpv !== undefined || body.rps !== undefined) {
    return NextResponse.json({ error: "Ranking points are read-only and are earned from analyzed-map passes only." }, { status: 400 });
  }
  ...
  if (body.title !== undefined || body.titleColor !== undefined || body.titleNeon !== undefined) {
    ...
    if (!title) await prisma.$executeRawUnsafe('DELETE FROM "UserProfileTitle" WHERE "userId"=$1', id);
    else await prisma.$executeRawUnsafe('INSERT INTO "UserProfileTitle" ("id","userId","title","color","neon","createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT ("userId") DO UPDATE SET "title"=EXCLUDED."title","color"=EXCLUDED."color","neon"=EXCLUDED."neon","updatedAt"=CURRENT_TIMESTAMP', randomUUID(), id, title, color, neon);
  }

  await prisma.moderationAction.create({ data: { actorId: owner.id, action: "owner_profile_inline_edited", targetType: "user", targetId: id, metadata: { ...data, title: typeof body.title === "string" ? body.title.trim().slice(0, 40) : null, titleColor: typeof body.titleColor === "string" ? body.titleColor.trim() : null, titleNeon: body.titleNeon === true } } });
