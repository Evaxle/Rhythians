...
import { markMapAnalysisFailed, saveMapAnalysis } from "@/lib/map-analysis-store";
  ...
  try {
    ...
    try {
      ...
      if (Number.isFinite(declared) && declared > 64 * 1024 * 1024) {
        ...
        analysis = await analyzeOversizedSspm(map.mapFileUrl);
      } else {
        const bytes = new Uint8Array(await response.arrayBuffer());
        if (!bytes.byteLength) throw new Error("Map source returned an empty file.");
        analysis = bytes.byteLength > 64 * 1024 * 1024 ? await analyzeOversizedSspm(map.mapFileUrl) : analyzeMapBytes(bytes);
      }
    } finally {
      clearTimeout(timeout);
    }
    await saveMapAnalysis(map.id, "legacy", analysis);
    return { id: map.id, ok: true as const };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Legacy map analysis failed.";
    await markMapAnalysisFailed(map.id, "legacy", message).catch(() => null);
...
export async function GET(request: Request) {
  ...
  const candidates = await prisma.$queryRawUnsafe<Candidate[]>(`
    ...
    WHERE c.status='legacy' AND c."mapFileUrl" IS NOT NULL
      AND ${retryFailed ? `(a.status='failed' AND a."sourceStatus"='legacy' AND a."analyzerVersion"=$1)` : `(a."mapId" IS NULL OR a."analyzerVersion"<>$1 OR a.status<>'analyzed' OR a."sourceStatus"<>'legacy')`}
