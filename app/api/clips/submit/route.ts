...
import { externalClipPath, externalThumbnailPath, validateExternalClipUrl, type ClipSourceType } from "@/lib/clip-source";
...
export async function POST(request: Request) {
  ...
  if (!["upload", "tiktok", "youtube", "twitch"].includes(sourceType)) return NextResponse.json({ error: "Invalid clip source." }, { status: 400 });
  ...
  await prisma.moderationAction.create({
    data: {
      ...
      targetType: "clip",
      targetId: clip.id,
      metadata: { title, songName: clip.songName, tagIds, sourceType, sourceUrl: canonicalSourceUrl },
    },
  });
