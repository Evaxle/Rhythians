...
export async function POST(request: Request) {
  ...
  const map = maps[0];
  if (!map || map.status !== "approved") return NextResponse.json({ error: "Map is not an approved Rhythians map." }, { status: 404 });
  ...
  const ranked = map.rating != null;
  const rankIndex = ranked ? getRankIndex(user.rhp) : -1;
  const [rangeMin, rangeMax] = ranked ? getRankRange(rankIndex) : [0, 0];
  const eligibleForRhp = ranked && map.rating! >= rangeMin && map.rating! <= rangeMax;
  const points = eligibleForRhp ? map.rhpOverride ?? calculateRhp(map.rating!, accuracy, speed, rankIndex, map.length) : 0;
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!ranked || !eligibleForRhp) return { counted: false, alreadyCompleted: false, duplicate: false, points: 0, rhp: user.rhp, recentId: insertedRecent[0]?.id ?? null };
  ...
  return NextResponse.json({ ok: true, duplicate: result.duplicate, counted: result.counted, alreadyCompleted: result.alreadyCompleted, recentScoreId: result.recentId, map: { id: map.id, title: map.title, rating: map.rating }, ranked, points: result.points, rhp: result.rhp, gameVersion: gameVersion || null, integrationVersion: integrationVersion || null });
}
