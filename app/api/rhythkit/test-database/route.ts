...
export async function POST(request: Request) {
  ...
  const installations = await prisma.$queryRawUnsafe<Array<{ userId: string }>>(
    `SELECT "userId" FROM "RhythKitInstallation" WHERE "tokenHash" = $1 AND "revokedAt" IS NULL LIMIT 1`,
    hashToken(token)
  );
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; created_at: Date }>>(
    `INSERT INTO public.rhythkit_connection_tests (user_id, source) VALUES ($1, 'RhythKit Agent') RETURNING id, created_at`,
    installation.userId
  );
