...
export async function POST(request: Request) {
  ...
  const refreshed = await getSeasonalPath();
  const assigned = refreshed.ranks.find((rank) => rank.index === rankIndex)?.map?.map ?? null;
  ...
  return NextResponse.json({ ok: true, rankIndex, map: { id: assigned.id, title: assigned.title, rating: assigned.rating } });
}
