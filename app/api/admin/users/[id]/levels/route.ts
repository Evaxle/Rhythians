...
export async function GET(_request: Request, { params }: Props) {
  ...
  const [challengeLevel, categoryLevels, titleRows] = await Promise.all([getUserChallengeLevel(id), getUserCategoryLevels(id), prisma.$queryRawUnsafe<Array<{ title: string; color: string; neon: boolean }>>('SELECT "title", "color", "neon" FROM "UserProfileTitle" WHERE "userId" = $1 LIMIT 1', id)]);
  return NextResponse.json({ challengeLevel, categoryLevels, profileTitle: titleRows[0]?.title ?? "", profileTitleColor: titleRows[0]?.color ?? "#a78bfa", profileTitleNeon: titleRows[0]?.neon ?? false, canEditTitle: isOwner(auth.admin) });
}
