...
export async function PATCH(request: Request, { params }: Props) {
  ...
  const body = (await request.json().catch(() => null)) as { title?: unknown; color?: unknown; neon?: unknown } | null;
  ...
  if (!title) {
    ...
    return NextResponse.json({ title: "", color: "#a78bfa", neon: false });
  }
  ...
  await prisma.moderationAction.create({ data: { actorId: admin.id, action: "profile_title_edited", targetType: "user", targetId: id, metadata: { title, color, neon } } });
  return NextResponse.json({ title, color, neon });
}
