...
import type { MapDifficultyDetails, MapPatternSegment, MapSectionAnalysis, MapSpeedProfile } from "@/lib/map-difficulty";
import type { Prisma } from "@/generated/prisma/client";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

type RawAnalysisRow = Omit<StoredMapAnalysis, "speedProfiles" | "topSections" | "patternSegments" | "analysisDetails"> & {
...
function normalizeAnalysis(row: RawAnalysisRow): StoredMapAnalysis {
  return {
    ...row,
    speedProfiles: jsonArray<MapSpeedProfile>(row.speedProfiles),
    topSections: jsonArray<unknown>(row.topSections),
    patternSegments: jsonArray<MapPatternSegment>(row.patternSegments),
    analysisDetails: jsonObject<MapDifficultyDetails>(row.analysisDetails),
  };
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const analysisRows = maps.length
      ? await prisma.$queryRawUnsafe<RawAnalysisRow[]>(
          `SELECT * FROM "MapDifficultyAnalysis" WHERE "mapId" IN (${maps.map((_, index) => `$${index + 1}`).join(",")})`,
          ...maps.map((map) => map.id),
        )
      : [];
    const analysisByMapId = new Map(analysisRows.map((row) => [row.mapId, normalizeAnalysis(row)]));

    const rows = maps.map((map) => {
      const analysis = analysisByMapId.get(map.id) ?? null;
    ...
    console.error("admin map catalog failed", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load map catalog." },
