...
export async function PATCH(request: Request, { params }: Props) {
  ...
  if (!["approve", "deny"].includes(action as string)) return NextResponse.json({ error: "Invalid action." }, { status: 400 });
  const requestRecord = await prisma.rhythiaProfileRequest.findUnique({ where: { id }, include: { user: { select: { id: true, profileHandle: true } } } });
  ...
  if (action === "approve") {
    ...
    const alreadyLinked = await prisma.rhythiaProfile.findUnique({ where: { profileId: requestRecord.profileId }, select: { userId: true } });
    if (alreadyLinked && alreadyLinked.userId !== requestRecord.userId) return NextResponse.json({ error: "That Rhythia profile is already linked to a different account." }, { status: 409 });
    await prisma.rhythiaProfile.upsert({ where: { userId: requestRecord.userId }, create: { userId: requestRecord.userId, profileUrl: requestRecord.profileUrl, ...profileData }, update: { profileUrl: requestRecord.profileUrl, ...profileData, syncedAt: now } });
    await prisma.user.update({ where: { id: requestRecord.userId }, data: { rhythiaVerified: true } });
    await prisma.rhythiaProfileRequest.update({ where: { id }, data: { status: "approved", adminNote: message || null, resolvedAt: now, resolvedBy: admin.id } });
    ...
    await prisma.notification.create({ data: { userId: requestRecord.userId, type: "moderation", title: "Rhythia profile link request denied", message: message || "Your request to link that Rhythia profile was denied.", url: `/profile/${requestRecord.user.profileHandle}` } });
  }
