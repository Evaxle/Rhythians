...
export async function PATCH(request: Request) {
  ...
  const requestRow = await prisma.rhythiaProfileRequest.findUnique({ where: { id }, include: { user: { select: { id: true, username: true, profileHandle: true, scoreImportDone: true } } } });
  ...
  if (action === "deny") {
    await prisma.$transaction([prisma.rhythiaProfileRequest.update({ where: { id }, data: { status: "denied", adminNote: note || requestRow.adminNote, resolvedAt: new Date(), resolvedBy: admin.id } }), prisma.notification.create({ data: { userId: requestRow.userId, type: "moderation", title: "Rhythia verification denied", message: note ? `Your Rhythia verification request was denied: ${note}` : "Your Rhythia verification request was denied.", url: "/settings" } })]);
  ...
  try {
    ...
    if (!requestRow.user.scoreImportDone) try { scoreImport = await rebuildRhythiaScorePoints(requestRow.userId); } catch {}
    return NextResponse.json({ ok: true, status: "approved", scoreImport });
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load the Rhythia profile." }, { status: 502 }); }
}
